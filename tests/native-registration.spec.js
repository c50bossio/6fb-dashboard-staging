...
test.describe('Native Registration Flow End-to-End', () => {
  ...
  test.beforeEach(async ({ page }) => {
    // Start from the plan selection page to test complete flow
  ...
  test('Complete native registration flow from plan selection to billing', async ({ page }) => {
    ...
    // Step 1: Select a plan and proceed to registration
    console.log('Step 1: Plan selection');
    await page.click('[data-plan="barber"]');
    await page.waitForTimeout(1000);
    
    // Check if we're redirected to registration with plan data
    ...
    // Step 2: Verify registration page loads with plan data
    ...
    // Step 3: Fill out Step 1 - Personal Information
    ...
    // Step 4: Fill out Step 2 - Business Information
    ...
    // Listen for response to check success
    ...
    // Step 6: Wait for success message and verify redirect
    ...
    // Wait for redirect to subscription page
    await page.waitForURL(/subscribe/, { timeout: 10000 });
    console.log('✅ Redirected to subscription/billing page');
    
    // Verify we're on the subscription page with source parameter
    ...
    // This will be verified separately via direct database query
  ...
  test.afterEach(async ({ page }) => {
    ...
    await page.evaluate(async () => {
      // This cleanup will be handled by direct database query
    });
  });
});