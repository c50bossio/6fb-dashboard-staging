...
class BarberNotificationManager {
  ...
  subscribe(barberbarbershopId, callback) {
    ...
    const staffChannel = this.supabase
      .channel(`barber_staff_${barberbarbershopId}`)
      .on(
        'postgres_changes',
        {
          event: '*',
  ...
  // Performance alerts
  async checkPerformanceAlerts(barberbarbershopId) {
    try {
...
export function useBarberNotifications(barberbarbershopId) {
  ...
  useEffect(() => {
    ...
    const subscriberId = barberNotifications.subscribe(barberbarbershopId, handleNotification)
