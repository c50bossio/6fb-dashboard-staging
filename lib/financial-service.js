...
class FinancialService {
   ...
   */
  async getArrangement(barberId, barberbarbershopId) {
   ...
   */
  async getShopArrangements(barberbarbershopId) {
    const supabase = this.getSupabase()
    if (!supabase) return { error: 'Client not initialized' }
  ...
  async saveArrangement(arrangementData) {
    ...
    try {
      ...
      if (!arrangementData.barberbarbershop_id || !arrangementData.barber_id) {
      ...
      const { data: existing } = await supabase
        ...
        .eq('barberbarbershop_id', arrangementData.barberbarbershop_id)
        .single()
      ...
      this.broadcastUpdate('arrangement_updated', {
        barberbarbershop_id: arrangementData.barberbarbershop_id,
   ...
   */
  async calculateCommission(amount, barberId, barberbarbershopId) {
    const { data: arrangement, error } = await this.getArrangement(barberId, barberbarbershopId)
    ...
    if (arrangement.use_tier_system && arrangement.tier_structure_id) {
      const tierCalculation = await this.calculateTieredCommission(amount, barberId, barberbarbershopId, arrangement)
   ...
   */
  async calculateTieredCommission(amount, barberId, barberbarbershopId, arrangement) {
    const supabase = this.getSupabase()
   ...
   */
  async getFinancialSummary(barberbarbershopId, dateRange = {}) {
    const supabase = this.getSupabase()
    if (!supabase) return { error: 'Client not initialized' }

    try {
      // Get all arrangements
      const { data: arrangements } = await this.getShopArrangements(barberbarbershopId)
  ...
  async recordTransaction(transactionData) {
    ...
    try {
      const { amount, barberId, barberbarbershopId, paymentIntentId, metadata } = transactionData

      // Calculate commission split with tier support
      const commission = await this.calculateCommission(amount, barberId, barberbarbershopId)
      ...
      await this.updateBarberBalance(barberId, barberbarbershopId, commission.barberAmount)
   ...
   */
  async updateBarberBalance(barberId, barberbarbershopId, amount) {
    const supabase = this.getSupabase()
    if (!supabase) return
   ...
   */
  async initializeStripeConnect(barberId, barberbarbershopId, accountData) {
    try {
  ...
  async createSplitPayment(paymentData) {
    const { amount, barberId, barberbarbershopId, customerId, serviceId } = paymentData

    try {
      // Get the barber's financial arrangement
      const { data: arrangement } = await this.getArrangement(barberId, barberbarbershopId)
   ...
   */
  async processPayout(barberId, barberbarbershopId, amount) {
    const supabase = this.getSupabase()
    if (!supabase) return { error: 'Client not initialized' }
   ...
   */
  subscribeToUpdates(barberbarbershopId, callback) {
    const supabase = this.getSupabase()
    if (!supabase) return null

    const subscription = supabase
      .channel(`financial_${barberbarbershopId}`)
      .on('postgres_changes', 
        { 
          event: '*', 
          schema: 'public', 
   ...
   */
  async getTierStructure(barberbarbershopId, structureId = null) {
    const supabase = this.getSupabase()
    if (!supabase) return { error: 'Client not initialized' }
   ...
   */
  async getBarberTierStatus(barberId, barberbarbershopId) {
    const supabase = this.getSupabase()
    if (!supabase) return { error: 'Client not initialized' }
   ...
   */
  async updateBarberTierProgress(barberId, barberbarbershopId, transactionAmount, tierInfo) {
    const supabase = this.getSupabase()
    if (!supabase) return { error: 'Client not initialized' }
   ...
   */
  async saveTierStructure(barberbarbershopId, structureData) {
    const supabase = this.getSupabase()
    if (!supabase) return { error: 'Client not initialized' }
   ...
   */
  async assignBarberToTierStructure(barberId, barberbarbershopId, structureId) {
    const supabase = this.getSupabase()
    if (!supabase) return { error: 'Client not initialized' }
   ...
   */
  async logTierAchievement(barberId, barberbarbershopId, tier, assignment) {
    const supabase = this.getSupabase()
    if (!supabase) return
  ...
  validateArrangement(data) {
    ...
    if (!data.barberbarbershop_id) errors.push('Barbershop ID is required')
   ...
   */
  async getProductCommissionCategories(barberbarbershopId) {
    const supabase = this.getSupabase()
    if (!supabase) return { error: 'Client not initialized' }
   ...
   */
  async saveProductCommissionCategory(barberbarbershopId, categoryData) {
    const supabase = this.getSupabase()
    if (!supabase) return { error: 'Client not initialized' }
   ...
   * @returns {Object} Commission calculation with tier information
   */
  async calculateProductCommission(saleData, barberId, barberbarbershopId) {
    const supabase = this.getSupabase()
    if (!supabase) return { error: 'Client not initialized' }
    ...
    try {
      ...
      const { data: categories } = await this.getProductCommissionCategories(barberbarbershopId)
   ...
   */
  async calculateProductTierAdjustment(tierContributionAmount, barberId, barberbarbershopId, arrangement) {
    const supabase = this.getSupabase()
    if (!supabase) return { success: false, error: 'Client not initialized' }
   ...
   */
  async recordProductCommissionTransactions(saleData, commissionCalculation, barberId, barberbarbershopId) {
    const supabase = this.getSupabase()
    if (!supabase) return { error: 'Client not initialized' }
   ...
   */
  async updateBarberProductTierProgress(barberId, barberbarbershopId, tierContributionAmount, tierInfo) {
    const supabase = this.getSupabase()
    if (!supabase) return
  ...
  async processProductReturn(returnData) {
    ...
    try {
      ...
      // Update barber balance
      await this.updateBarberBalance(barberId, barberbarbershopId, totalCommissionAdjustment)

      // Update tier progress if there's a tier adjustment
      if (Math.abs(totalTierAdjustment) > 0) {
        await this.adjustBarberTierProgress(barberId, barberbarbershopId, totalTierAdjustment)
      }
   ...
   */
  async adjustBarberTierProgress(barberId, barberbarbershopId, adjustmentAmount) {
    const supabase = this.getSupabase()
    if (!supabase) return
   ...
   */
  async getComprehensiveCommissionSummary(barberbarbershopId, dateRange = {}) {
    const supabase = this.getSupabase()
    if (!supabase) return { error: 'Client not initialized' }
   ...
   */
  async getRealtimeFinancialMetrics(barberbarbershopId, dateRange = {}) {
    const supabase = this.getSupabase()
    if (!supabase) return { error: 'Client not initialized' }
   ...
   */
  async getTierProgressionAnalytics(barberbarbershopId, dateRange = {}) {
    const supabase = this.getSupabase()
    if (!supabase) return { error: 'Client not initialized' }
   ...
   */
  async initializeDefaultProductCategories(barberbarbershopId) {
    ...
    for (const category of defaultCategories) {
      const result = await this.saveProductCommissionCategory(barberbarbershopId, category)
      results.push(result)
    }
