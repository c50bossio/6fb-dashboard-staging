...
export async function getCachedResults(barbershopId, priority, daysSinceVisit, includeNoShows) {
  ...
  try {
    ...
    if (cachedData) {
      ...
      console.log(`Cache hit for barbershop ${barbershopId}, TTL: ${data.cached.expires_in_seconds}s`)
      return data
      
    } else {
      cacheStats.misses++
      console.log(`Cache miss for barbershop ${barbershopId}`)
      return null
    }
...
export async function cacheResults(barbershopId, priority, daysSinceVisit, includeNoShows, data) {
  ...
  try {
    ...
    console.log(`Cached data for barbershop ${barbershopId} (TTL: ${ttl/60}min, ${storeTime}ms)`)
    return true
...
export async function invalidateClientCareCache(barbershopId, reason = 'data_change') {
  ...
  try {
    ...
    if (keys.length > 0) {
      ...
      console.log(`Cleared ${keys.length} cache keys for barbershop ${barbershopId}`)
    }
