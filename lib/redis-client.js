 ...
 */
export function generateCacheKey(barberbarbershopId, priority = 'all', daysSinceVisit = 60, includeNoShows = true) {
  const keyComponents = [
    'client-care',
 ...
 */
export async function getCachedResults(barberbarbershopId, priority, daysSinceVisit, includeNoShows) {
  if (!redisClient) {
  ...
  try {
    const cacheKey = generateCacheKey(barberbarbershopId, priority, daysSinceVisit, includeNoShows)
    ...
    if (cachedData) {
      ...
      console.log(`Cache hit for barbershop ${barberbarbershopId}, TTL: ${data.cached.expires_in_seconds}s`)
      return data
      
    } else {
      cacheStats.misses++
      console.log(`Cache miss for barbershop ${barberbarbershopId}`)
      return null
    }
 ...
 */
export async function cacheResults(barberbarbershopId, priority, daysSinceVisit, includeNoShows, data) {
  if (!redisClient) {
    redisClient = initRedis()
  }
  ...
  try {
    ...
    console.log(`Cached data for barbershop ${barberbarbershopId} (TTL: ${ttl/60}min, ${storeTime}ms)`)
    return true
 ...
 */
export async function invalidateClientCareCache(barberbarbershopId, reason = 'data_change') {
  if (!redisClient) {
    redisClient = initRedis()
  }
  ...
  try {
    ...
    const pattern = `client-care:shop:${barberbarbershopId}:*`
    const keys = await redisClient.keys(pattern)
    
    if (keys.length > 0) {
      await redisClient.del(...keys)
      console.log(`Cleared ${keys.length} cache keys for barbershop ${barberbarbershopId}`)
    }
