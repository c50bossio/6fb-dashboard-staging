...
export class AutomationOrchestrator {
      ...
      if (allSettings) {
        allSettings.forEach(setting => {
          ...
          if (automation && setting.profiles?.barbershop_id) {
            const barbershopId = setting.profiles.barbershop_id
            
            this.settings[barbershopId] = {
              ...automation,
  ...
  async handleNoShowEvent(data) {
    const { appointmentId, barbershopId, clientId } = data
    const settings = this.settings[barbershopId]
  ...
  async handlePaymentFailed(data) {
    const { paymentId, appointmentId, barbershopId, clientId, reason } = data
    const settings = this.settings[barbershopId]
  ...
  async handleClientBlocked(data) {
    const { clientId, barbershopId, reason } = data
    const settings = this.settings[barbershopId]
  ...
  async notifyManager(data) {
    try {
      const { type, barbershopId, settings } = data
   ...
   */
  getAutomationStatus(barbershopId) {
    const settings = this.settings[barbershopId]
