...
class AdaptiveRateLimiter {
  ...
  async checkRateLimit(feature, context = {}) {
    ...
    if (adaptedConfig.perBarbershop && context.barberbarbershop_id) {
      const limiter = new SlidingWindowRateLimiter(
        feature, 
        RATE_LIMIT_TYPES.PER_BARBERSHOP, 
        context.barberbarbershop_id, 
  ...
  async getRateLimitStatus(feature, context = {}) {
    ...
    if (config.perBarbershop && context.barberbarbershop_id) {
      const limiter = new SlidingWindowRateLimiter(
        feature, 
        RATE_LIMIT_TYPES.PER_BARBERSHOP, 
        context.barberbarbershop_id, 
...
export const checkAutomationRateLimit = async (feature, context) => {
  ...
  if (context.barberbarbershop_id) {
    overrides.push(rateLimiter.hasOverride(feature, context.barberbarbershop_id, RATE_LIMIT_TYPES.PER_BARBERSHOP));
  }
