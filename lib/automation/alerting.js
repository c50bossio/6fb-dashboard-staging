...
class AlertingSystem extends EventEmitter {
  ...
  async sendAlert(alertType, alertData, options = {}) {
    ...
    try {
      ...
      const alert = {
        ...
        details: alertData,
        timestamp: new Date(),
        barberbarbershop_id: alertData.barberbarbershop_id,
  ...
  async isAlertSuppressed(alertId, alertType) {
    try {
      ...
      const isSpecificallySupp
        return await redisClient.exists(alertSuppressionKey) === 1;
...
export { AlertingSystem, ALERT_SEVERITY, ALERT_CATEGORIES, ALERT_CHANNELS };