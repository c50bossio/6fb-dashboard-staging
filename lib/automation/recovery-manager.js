...
export class RecoveryManagerService {
  ...
  async startRecoveryFlow(data) {
    const { barberbarbershopId, clientId, trigger, settings, appointmentId, reason } = data
  ...
  async executeManagerContactStep(recovery, step) {
    const managers = await this.getBarbershopManagers(recovery.barberbarbershopId)
    if (!managers?.length) {
    ...
    const task = await this.createManagerTask({
      ...
      barberbarbershopId: recovery.barberbarbershopId,
  ...
  async saveRecoveryState(recovery) {
    try {
      ...
      await supabase
        ...
        .upsert({
          id: recovery.id,
          barberbarbershop_id: recovery.barberbarbershopId,
