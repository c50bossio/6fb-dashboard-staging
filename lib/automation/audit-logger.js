...
class AuditLogger extends EventEmitter {
    ...
    try {
      ...
      const auditEntry = {
        ...
        resource: {
          ...
          barberbarbershop_id: options.barberbarbershopId,
  ...
  async logUserAction(user, action, resource, eventData, options = {}) {
    return this.logEvent(action, eventData, {
      ...
      barberbarbershopId: resource.barberbarbershop_id
    });
  }
  ...
  async handleHighSeverityEvent(auditEntry) {
    try {
      await sendAlert('high_severity_audit_event', {
        ...
        timestamp: auditEntry.timestamp,
        barberbarbershop_id: auditEntry.resource.barberbarbershop_id
      });
    } catch (error) {
  ...
  async storeInRedis(entries) {
    try {
      ...
      for (const entry of entries) {
        ...
        if (entry.resource.barberbarbershop_id) {
          const shopKey = `audit_shop:${entry.resource.barberbarbershop_id}`;
  ...
  async storeInDatabase(entries) {
    try {
      ...
      const dbEntries = entries.map(entry => ({
        ...
        barberbarbershop_id: entry.resource.barberbarbershop_id,
  ...
  async createAuditIndexes() {
    try {
      ...
      const indexes = [
        ...
        'CREATE INDEX IF NOT EXISTS idx_audit_logs_barberbarbershop_id ON audit_logs(barberbarbershop_id)',
  ...
  async searchAuditLogs(filters = {}, options = {}) {
    try {
      ...
      if (filters.barberbarbershopId) {
        query = query.eq('barberbarbershop_id', filters.barberbarbershopId);
      }
