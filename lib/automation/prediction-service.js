...
export class PredictionService {
  ...
  async getWeatherFeatures(appointmentData) {
    try {
      ...
      const location = await this.getBarbershopLocation(appointmentData.barberbarbershop_id)
  ...
  async getTimeFeatures(appointmentData) {
    try {
      ...
      const historicalRates = await this.getHistoricalNoShowRates(appointmentData.barberbarbershop_id, dayOfWeek, hour)
  ...
  async getServiceFeatures(appointmentData) {
    try {
      ...
      const serviceStats = await this.getServiceStats(service.id, appointmentData.barberbarbershop_id)
  ...
  async scheduleExtraReminder(appointmentData, riskPrediction) {
    try {
      ...
      await this.orchestrator.services.reminderEngine.scheduleReminders({
        ...
        barberbarbershopId: appointmentData.barberbarbershop_id,
  ...
  async alertWaitlist(appointmentData, riskPrediction) {
    try {
      ...
      const { data: waitlistEntries } = await supabase
        .from('waitlist')
        .select('*')
        .eq('barberbarbershop_id', appointmentData.barberbarbershop_id)
  ...
  async getBarbershopLocation(barberbarbershopId) {
