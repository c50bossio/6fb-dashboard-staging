...
import { createClient } from './supabase/UNIFIED_CLIENT'

// Create supabase client for analytics operations
...
class AnalyticsJobQueue {
   ...
   */
  async calculateHealthScores({ barberbarbershopId, customerIds = [] }) {
    const batchSize = 50
    const results = []
    ...
    // Invalidate cache
    analyticsCache.invalidate(`health_scores:${barberbarbershopId}`)
   ...
   */
  async calculateCLV({ barberbarbershopId, customerIds = [] }) {
    const results = []
    ...
    for (const customerId of customerIds) {
      ...
      // Calculate metrics
      const totalRevenue = appointments.reduce((sum, a) => sum + (a.total_price_cents ? a.total_price_cents / 100 : 0), 0)
    ...
    // Invalidate cache
    analyticsCache.invalidate(`clv:${barberbarbershopId}`)
   ...
   * Analyze churn risk
   */
  async analyzeChurnRisk({ barberbarbershopId, customerIds = [] }) {
    const results = []
   ...
   */
  async generateInsights({ barberbarbershopId, timeframe = 'month' }) {
    ...
    const insights = {
      summary: {
        ...
        total_revenue: appointments?.reduce((sum, a) => sum + (a.total_price_cents ? a.total_price_cents / 100 : 0), 0) || 0,
   ...
   * Segment customers
   */
  async segmentCustomers({ barberbarbershopId }) {
...
export function scheduleHealthScoreCalculation(barberbarbershopId, customerIds) {
  return analyticsJobQueue.enqueue({
    type: 'calculate_health_scores',
    params: { barberbarbershopId, customerIds },
    priority: 7
  })
}

export function scheduleCLVCalculation(barberbarbershopId, customerIds) {
  return analyticsJobQueue.enqueue({
    type: 'calculate_clv',
    params: { barberbarbershopId, customerIds },
    priority: 6
...
export function scheduleChurnAnalysis(barberbarbershopId, customerIds) {
  return analyticsJobQueue.enqueue({
...
export function scheduleInsightsGeneration(barberbarbershopId, timeframe) {
  return analyticsJobQueue.enqueue({
    type: 'generate_insights',
    params: { barberbarbershopId, timeframe },
    priority: 5
  })
}

export function scheduleCustomerSegmentation(barberbarbershopId) {
  return analyticsJobQueue.enqueue({
    type: 'segment_customers',
    params: { barberbarbershopId },
    priority: 4
  })
}
