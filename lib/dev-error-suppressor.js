...
class DevErrorSuppressor {
  constructor() {
    ...
    this.commonNoisePatterns = [
      ...
      // BookedBarber debug logs (suppress excessive development logging)
      '🏪 BusinessContext:',
      '🏪 BookedBarber:',
      '🔐 AUTH PROVIDER:',
      '🔐 Auth:',
      ...
      'Component function executing',
      'State initialized',
      'Hydration effect',
      'Main auth effect',
      'Overall loading state',
      'Computing business context',
      'Hook returning',
      'Context value being provided',
  ...
  // Show stats in development console (silent by default)
  showStats() {
    // Only show if explicitly enabled via debug flag
    if (process.env.NODE_ENV !== 'development' || !process.env.NEXT_PUBLIC_DEBUG_ERROR_SUPPRESSOR) {
      return
    }
...
if (process.env.NODE_ENV === 'development' && typeof window !== 'undefined') {
  ...
  // Show stats only if debug flag is enabled
  if (process.env.NEXT_PUBLIC_DEBUG_ERROR_SUPPRESSOR) {
    setInterval(() => {
