...
export async function trackAIUsage(userId, provider, tokens, metadata = {}) {
  try {
    console.log(`🤖 [AI USAGE] User ${userId}: ${tokens} tokens via ${provider}`)
...
export async function trackSMSUsage(userId, recipient, messageType = 'general', metadata = {}) {
  try {
    console.log(`📱 [SMS USAGE] User ${userId}: SMS sent to ${recipient?.slice(-4) || 'unknown'} (${messageType})`)
...
export async function trackEmailUsage(userId, recipient, emailType = 'general', metadata = {}) {
  try {
    console.log(`📧 [EMAIL USAGE] User ${userId}: Email sent to ${recipient || 'unknown'} (${emailType})`)
...
export async function checkUsageLimitsForUser(userId) {
  try {
    ...
    if (warnings.length > 0) {
      console.log(`📊 [USAGE CHECK] User ${userId} has ${warnings.length} usage warnings:`, warnings)
