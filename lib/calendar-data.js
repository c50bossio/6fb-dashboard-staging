...
// Empty state placeholder when no real barbers are configured
export const EMPTY_BARBER_PLACEHOLDER = [
  {
    ...
    extendedProps: {
      isPlaceholder: true,
      isEmpty: true,
      message: 'Add your first barber to start accepting bookings'
    }
  }
]

// Default resources - should be populated from real data
...
export async function fetchRealEvents(startDateParam = null, endDateParam = null, resources = DEFAULT_RESOURCES, services = DEFAULT_SERVICES, barbershopId = null) {
  const events = []
  
  // Validate required barbershopId parameter
  if (!barbershopId) {
    console.warn('fetchRealEvents: No barbershopId provided, cannot fetch appointments')
    return events
  }
  ...
  if (startDateParam && endDateParam) {
    dateRange.startDate = new Date(startDateParam).toISOString()
    dateRange.endDate = new Date(endDateParam).toISOString()
  } else {
    ...
    dateRange.endDate = end.toISOString()
  }
  
  try {
    const response = await fetch(`/api/calendar/appointments?barbershop_id=${barbershopId}&start_date=${dateRange.startDate}&end_date=${dateRange.endDate}`)
...
export async function fetchRecurringEvents(date = new Date(), barbershopId = null) {
  const recurringEvents = []
  
  // Validate required barbershopId parameter
  if (!barbershopId) {
    console.warn('fetchRecurringEvents: No barbershopId provided, cannot fetch recurring appointments')
    return recurringEvents
  }
  
  try {
    const response = await fetch(`/api/calendar/recurring?barbershop_id=${barbershopId}`)
...
// Fetch real barbers for a specific barbershop
export async function fetchRealBarbers(barbershopId = null) {
  if (!barbershopId) {
    console.warn('fetchRealBarbers: No barbershopId provided, using empty placeholder')
    return EMPTY_BARBER_PLACEHOLDER
  }

  try {
    const response = await fetch(`/api/shop/barbers?barbershop_id=${barbershopId}`)
    ...
    if (response.ok) {
      ...
      if (barbers.length === 0) {
        console.info('No barbers found for shop, showing empty state')
        return EMPTY_BARBER_PLACEHOLDER
      }
      
      // Transform barbers to calendar resources
      return barbers.map(barber => ({
        id: barber.id,
        title: barber.name || barber.full_name || 'Barber',
        eventColor: barber.calendar_color || '#546355',
        extendedProps: {
          email: barber.email,
          phone: barber.phone,
      ...
      console.error('Failed to fetch barbers from database')
      return EMPTY_BARBER_PLACEHOLDER
    }
  } catch (error) {
    console.error('Error fetching real barbers:', error)
    return EMPTY_BARBER_PLACEHOLDER
  }
}
