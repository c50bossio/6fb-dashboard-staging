...
export async function fetchRealEvents(date = new Date(), resources = DEFAULT_RESOURCES, services = DEFAULT_SERVICES) {
  ...
  try {
    ...
    if (response.ok) {
      ...
      // Transform database appointments to calendar events
      appointments.forEach(appointment => {
        ...
        events.push({
          ...
          backgroundColor: appointment.backgroundColor || resource.eventColor || '#3b82f6',
          borderColor: appointment.borderColor || resource.eventColor || '#3b82f6',
...
// Fetch real recurring appointments from database
export async function fetchRecurringEvents(date = new Date()) {
  const recurringEvents = []
  
  try {
    // Fetch recurring appointments from API
    ...
    if (response.ok) {
      ...
      // Transform database recurring appointments to calendar events
