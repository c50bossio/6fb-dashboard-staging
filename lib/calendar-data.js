...
export async function fetchRealEvents(startDateParam = null, endDateParam = null, resources = DEFAULT_RESOURCES, services = DEFAULT_SERVICES, shopId = null) {
  const events = []
  
  // Validate required shopId parameter
  if (!shopId) {
    console.warn('fetchRealEvents: No shopId provided, cannot fetch appointments')
    return events
  }
  ...
  try {
    const response = await fetch(`/api/calendar/appointments?barbershop_id=${shopId}&start_date=${startDate}&end_date=${endDate}`)
...
export async function fetchRecurringEvents(date = new Date(), shopId = null) {
  const recurringEvents = []
  
  // Validate required shopId parameter
  if (!shopId) {
    console.warn('fetchRecurringEvents: No shopId provided, cannot fetch recurring appointments')
...
export async function fetchRealBarbers(shopId = null) {
  if (!shopId) {
    console.warn('fetchRealBarbers: No shopId provided, using empty placeholder')
    return EMPTY_BARBER_PLACEHOLDER
  }

  try {
    const response = await fetch(`/api/shop/barbers?shop_id=${shopId}`)
