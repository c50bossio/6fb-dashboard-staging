...
export function generateMockEvents(date = new Date(), resources = DEFAULT_RESOURCES, services = DEFAULT_SERVICES) {
  ...
  resources.forEach((resource, resourceIdx) => {
    ...
    for (let i = 0; i < appointmentCount; i++) {
      ...
      events.push({
        id: `mock-appointment-${resource.id}-${i}-${Date.now()}`, // Clear mock data identifier
        title: `${customer} - ${service.name}`,
        start: start, // Pass Date object directly, let FullCalendar handle timezone
        end: end, // Pass Date object directly
        ...
        extendedProps: {
          ...
          notes: 'Mock data - Regular customer',
          isMockData: true // Clear indicator this is mock data
        }
      })
    }
  })
...
export function generateRecurringEvents(date = new Date()) {
  ...
  const recurringEvents = [
    {
      id: `mock-recurring-1-${Date.now()}`, // Clear mock data identifier
      title: 'Mr. Thompson - Weekly Trim',
      rrule: {
        freq: 'weekly',
        byweekday: ['tu'],
        dtstart: new Date(baseDate.setHours(10, 0, 0, 0)) // Pass Date object
      },
      duration: '00:30',
      resourceId: 'barber-1',
      ...
      extendedProps: {
        ...
        isRecurring: true,
        frequency: 'Weekly',
        notes: 'Mock data - Regular customer - Every Tuesday',
        isMockData: true // Clear indicator this is mock data
      }
    },
    {
      id: `mock-recurring-2-${Date.now()}`, // Clear mock data identifier
      title: 'Joe Martinez - Beard Maintenance',
      rrule: {
        freq: 'weekly',
        byweekday: ['th'],
        dtstart: new Date(baseDate.setHours(14, 0, 0, 0)) // Pass Date object
      },
      duration: '00:45',
      resourceId: 'barber-2',
      ...
      extendedProps: {
        ...
        isRecurring: true,
        frequency: 'Weekly',
        notes: 'Mock data - Beard trim and shape - Every Thursday',
        isMockData: true // Clear indicator this is mock data
      }
    }
  ]
