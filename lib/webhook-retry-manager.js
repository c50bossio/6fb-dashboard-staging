...
class WebhookRetryManager {
   ...
   */
  async retryBalanceUpdate(barberId, barbershopId, amount, transactionId, supabase) {
    return await this.withRetry(
      async () => {
        return await this.updateBalanceAtomic(barberId, barbershopId, amount, transactionId, supabase)
      },
      {
   ...
   */
  async updateBalanceAtomic(barberId, barbershopId, amount, transactionId, supabase) {
  ...
  async createDeadLetterRecord(eventType, eventData, failureReason) {
    try {
      ...
      console.log(`📝 Dead letter queue record created for ${eventType}`)
    } catch (error) {
  ...
  async processDeadLetterQueue() {
    try {
      ...
      for (const item of queueItems) {
        console.log(`🔄 Processing dead letter item: ${item.event_type}`)
        ...
        try {
          ...
          if (success) {
            ...
            console.log(`✅ Successfully reprocessed dead letter item: ${item.id}`)
          } else {
            await this.supabase
              .from('webhook_dead_letter_queue')
              .update({ 
  ...
  async reprocessWebhookEvent(eventType, eventData) {
    try {
      ...
      switch (eventType) {
        ...
        default:
          console.log(`Unsupported event type for reprocessing: ${eventType}`)
          return false
      }
    } catch (error) {
