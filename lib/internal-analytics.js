...
class InternalAnalytics {
  ...
  async init() {
    try {
      ...
      if (!this.supabase) {
        console.warn('Analytics disabled: Supabase client not available')
        this.disabled = true
        return
      }
      
      // Analytics re-enabled - anon key has been fixed
      
      this.sessionId = getSessionId()
      
      // Get current user - wrap in try/catch to handle auth failures
      try {
        ...
        console.debug('Analytics: Unable to get user', error.message)
      }
      
      // Skip retry of failed events if disabled
      if (!this.disabled) {
  ...
  async track(eventName, eventData = {}) {
    if (!this.supabase || typeof window === 'undefined' || this.disabled) return
  ...
  async retryFailedEvents() {
    if (this.disabled) return // Skip if analytics is disabled
