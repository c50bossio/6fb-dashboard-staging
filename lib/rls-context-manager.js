...
export class RLSContextManager {
   ...
   * @param {string} context.shopId - Shop/barbershop ID
   * @param {string} context.role - User role (barber, shop_owner, etc.)
   * @param {Array} context.permissions - User permissions
   */
  async setContext(context) {
    try {
      const { userId, shopId, role, permissions = [] } = context
      
      if (!userId || !shopId) {
        throw new Error('userId and shopId are required for RLS context')
      }
   ...
   * @param {string} shopId - Shop ID to validate access to
   * @returns {Promise<boolean>} True if user has access
   */
  async validateShopAccess(userId, shopId) {
    try {
      ...
      if (profile?.shop_id === shopId || profile?.barbershop_id === shopId) {
      ...
      rlsLogger.warn('Shop access denied', { userId, shopId })
      return false
  ...
  async getUserAccessibleShops(userId) {
    try {
      ...
      staffShops?.forEach(staff => {
        // Avoid duplicates
        if (!accessibleShops.find(shop => shop.shopId === staff.barbershop_id)) {
          accessibleShops.push({
            shopId: staff.barbershop_id,
            role: staff.role,
            accessType: 'staff'
          })
        }
  ...
  createShopScopedQuery(tableName, context = null) {
    ...
    try {
      ...
      // Basic validation - just check if we have a query object
      // Note: Supabase query builders are Proxy objects, so detailed method checking can be unreliable

      // Apply shop filtering based on table structure
      switch (tableName) {
        case 'appointments':
        case 'customers':
          query = query.eq('barbershop_id', activeContext.shopId)
          break
        
        case 'services':
        case 'staff_schedules':
          query = query.eq('shop_id', activeContext.shopId)
          break
        ...
        case 'barbershops':
          query = query.eq('id', activeContext.shopId)
          break
        
        default:
          rlsLogger.warn('Unknown table for RLS scoping, using shop_id', { 
            tableName, 
            shopId: activeContext.shopId 
          })
          // Apply generic shop_id filter as fallback
          query = query.eq('shop_id', activeContext.shopId)
      }
    ...
    return async (...args) => {
      ...
      try {
        ...
        if (error.message?.includes('row-level security') || 
          ...
          throw new Error(`Access denied: You don't have permission to access this data (Shop: ${context.shopId})`)
        }
...
export const validateShopAccess = (userId, shopId) => rlsManager.validateShopAccess(userId, shopId)
