  ...
  async getUserAccessibleShops(userId) {
    try {
      ...
      staffShops?.forEach(staff => {
        // Avoid duplicates
        if (!accessibleShops.find(shop => shop.barbershopId === staff.barberbarbershop_id)) {
          accessibleShops.push({
            barbershopId: staff.barberbarbershop_id,
            role: staff.role,
            accessType: 'staff'
          })
        }
  ...
  createShopScopedQuery(tableName, context = null) {
    ...
    try {
      ...
      switch (tableName) {
        case 'appointments':
        case 'customers':
          query = query.eq('barberbarbershop_id', activeContext.barbershopId)
          break
...
export async function initializeRLSContext(session) {
  ...
  try {
    ...
    if (!barbershopId) {
      ...
      if (staffData) {
        barbershopId = staffData.barberbarbershop_id
      }
    }
