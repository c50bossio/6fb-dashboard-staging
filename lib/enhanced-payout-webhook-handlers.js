...
export async function enhancedHandleTransferCreated(transfer) {
  try {
    ...
    const barberbarbershopId = transfer.metadata?.barberbarbershop_id
    
    if (!barberbarbershopId) {
      console.warn('Transfer created without barberbarbershop_id metadata:', transfer.id)
      return { success: false, reason: 'missing_barberbarbershop_id' }
    }
    ...
    await upsertPayoutTransactionMetadata(payoutRecordId, barberbarbershopId, {
      stripe_transfer_id: transfer.id,
...
export async function enhancedHandleTransferPaid(transfer) {
  try {
    ...
    const { data: payoutRecord, error: payoutError } = await supabase
      .from('commission_payout_records')
      .select('id, barberbarbershop_id, barber_id, amount')
    ...
    await supabase
      ...
      .eq('barberbarbershop_id', payoutRecord.barberbarbershop_id)

    // Update transaction metadata with completion details
    await upsertPayoutTransactionMetadata(payoutRecordId, payoutRecord.barberbarbershop_id, {
    ...
    await triggerPerformanceMetricsUpdate(payoutRecord.barberbarbershop_id, supabase)

    // Send completion notification to barber
    await sendPayoutCompletionNotification({
      barberId: payoutRecord.barber_id,
      barberbarbershopId: payoutRecord.barberbarbershop_id,
...
export async function enhancedHandleTransferFailed(transfer) {
  try {
    ...
    await supabase
      ...
      .insert({
        payout_record_id: payoutRecordId,
        barberbarbershop_id: payoutRecord.barberbarbershop_id,
        barber_id: payoutRecord.barber_id,
...
async function upsertPayoutTransactionMetadata(payoutRecordId, barberbarbershopId, updates, supabase) {
  try {
...
async function triggerPerformanceMetricsUpdate(barberbarbershopId, supabase) {
  try {
