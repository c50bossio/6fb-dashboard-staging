...
export async function logGMBAuditEvent({
  ...
  try {
    ...
    if (error) {
      ...
      console.log('GMB Audit Entry:', JSON.stringify(auditEntry))
    }

    // Also maintain recent activity cache for quick access
    await cacheRecentActivity(barberbarbershop_id, auditEntry)
 ...
 */
async function cacheRecentActivity(barberbarbershopId, auditEntry) {
  try {
    const cacheKey = `gmb_recent_${barberbarbershopId}`
 ...
 */
export async function getGMBAuditSummary(barberbarbershopId, startDate, endDate) {
  try {
 ...
 */
export async function generateComplianceReport(barberbarbershopId, reportType = 'full') {
  try {
    const endDate = new Date().toISOString()
    const startDate = new Date(Date.now() - 30 * 24 * 60 * 60 * 1000).toISOString() // Last 30 days

    const auditSummary = await getGMBAuditSummary(barberbarbershopId, startDate, endDate)
