...
class Cin7RealtimeSync extends EventEmitter {
   ...
   */
  async initialize() {
    console.log('🚀 Initializing CIN7 Real-time Sync Service...')
    ...
    try {
      ...
      console.log('✅ CIN7 Real-time Sync Service initialized')
      return true
    } catch (error) {
  ...
  async processWebhook(webhookData, signature) {
    try {
      console.log(`📨 Processing CIN7 webhook: ${webhookData.Type}`)
  ...
  async handleStockUpdate(webhookData) {
    console.log(`📊 Processing stock update for product ${webhookData.ProductID}`)
    ...
    try {
      ...
      if (data && data.length > 0) {
        ...
        console.log(`✅ Updated stock for ${product.name}: ${product.current_stock}`)
  ...
  async handleProductUpdate(webhookData) {
    console.log(`📦 Processing product update for ${webhookData.ProductID}`)
    ...
    try {
      ...
      if (data && data.length > 0) {
        console.log(`✅ Updated product: ${data[0].name}`)
  ...
  async handleSaleCompleted(webhookData) {
    console.log(`💰 Processing completed sale: ${webhookData.SaleID}`)
    ...
    try {
      ...
      if (webhookData.Lines && Array.isArray(webhookData.Lines)) {
        for (const lineItem of webhookData.Lines) {
          ...
          if (data && data.length > 0) {
            const product = data[0]
            console.log(`📉 Reduced stock for ${product.name}: ${product.current_stock}`)
  ...
  async updateBookingAvailability(product) {
    console.log(`📅 Updating booking availability for ${product.name}`)
  ...
  startPeriodicSync() {
    ...
    this.syncInterval = setInterval(async () => {
      console.log('🔄 Running periodic inventory sync...')
  ...
  async performPeriodicSync() {
    ...
    try {
      ...
      if (!connections || connections.length === 0) {
        console.log('📝 No auto-sync connections found')
        return
      }

      console.log(`🔄 Syncing ${connections.length} barbershops...`)
  ...
  startWebhookRetryProcessor() {
    setInterval(async () => {
      if (this.webhookRetryQueue.length > 0) {
        console.log(`🔄 Processing ${this.webhookRetryQueue.length} webhook retries...`)
  ...
  async processWebhookRetries() {
    ...
    for (const retry of retries) {
      ...
      try {
        ...
        this.retryAttempts.delete(retry.id)
        console.log(`✅ Webhook ${retry.id} processed on retry ${attemptCount + 1}`)
  ...
  addToWebhookRetryQueue(webhookData, signature, error) {
    ...
    console.log(`📝 Added webhook ${retryId} to retry queue`)
  }
  ...
  async setupRealtimeListeners() {
    ...
    console.log('👂 Real-time database listeners active')
  ...
  shutdown() {
    ...
    console.log('🛑 CIN7 Real-time Sync Service stopped')
  }
}
