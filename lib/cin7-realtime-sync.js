...
class Cin7RealtimeSync extends EventEmitter {
  constructor() {
    super()
    this.syncQueue = new Map() // barberbarbershopId -> sync operations
...
const supabase = createClient()
    ...
    try {
      ...
      const { data, error } = await supabase
        ...
        .select('id, barberbarbershop_id, name, current_stock, min_stock_level')
...
const supabase = createClient()
    ...
    try {
      ...
      const mappedProduct = await this.mapCin7ProductToInventory(productData, connection.barberbarbershopId)
...
const supabase = createClient()
    ...
    try {
      ...
      const { error } = await supabase
        ...
        .insert({
          barberbarbershop_id: product.barberbarbershop_id,
      ...
      if (error) {
        ...
        this.emit('lowStockAlert', {
          barberbarbershopId: product.barberbarbershop_id,
  ...
  async performPeriodicSync() {
    ...
    try {
      ...
      const { data: connections } = await supabase
        ...
        .select('id, barberbarbershop_id, sync_settings')
        .eq('is_active', true)
...
// Import sync service
      ...
      for (const connection of connections) {
        try {
          await syncService.syncInventory(connection.barberbarbershop_id, {
          ...
          console.error(`❌ Periodic sync failed for barbershop ${connection.barberbarbershop_id}:`, syncError)
        }
      }
...
return inventoryChannel
  ...
  async getConnectionByProductId(supabase, productId) {
    ...
    const { data: connection } = await supabase
      ...
      .eq('barberbarbershop_id', inventory.barberbarbershop_id)
      .eq('is_active', true)
      .single()
    ...
    return {
      id: connection.id,
      barberbarbershopId: inventory.barberbarbershop_id,
