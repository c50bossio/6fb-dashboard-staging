...
export const CALENDAR_VIEWS = {
  ...
  // BARBER role includes team coordination views following industry best practices.
  // Research shows industry-standard barbershop software (Squire, Booksy) allows
  // barbers to view all team schedules by default for coordination and customer service.
  BARBER: [
    'book-appointment',
    'choose-barber',
    'choose-time',
    ...
    'my-clients',
    'availability',
    'all-barbers',      // View all team schedules (read-only)
    'shop-calendar'     // View shop-wide calendar (read-only)
  ],
  'BARBER & MANAGER': [
    'book-appointment',
    'choose-barber',
...
export async function getBarbersForLocations(locationIds, userRole) {
  ...
  try {
    ...
    const barbershopIds = [...new Set(staffRecords.map(s => s.barbershop_id).filter(Boolean))]
    ...
    const barbers = staffRecords.map(staff => {
      ...
      const barbershop = barbershops.find(b => b.id === staff.barbershop_id)
