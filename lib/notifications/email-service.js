...
// Import nodemailer only if available
let nodemailer = null;
try {
...
class EmailService {
  ...
  async sendEmail({ to, subject, html, text }) {
    ...
    const mailOptions = {
      from: `${process.env.FROM_NAME || process.env.SENDGRID_FROM_NAME || '6FB AI Agent System'} <${process.env.FROM_EMAIL || process.env.SENDGRID_FROM_EMAIL || 'support@em3014.6fbmentorship.com'}>`,
      to,
      subject,
      html,
