// Real Calendar Database Operations
// Replaces all mock event generators in calendar-data.js

import { createClient } from './supabase/server'

// ==========================================
// REAL APPOINTMENT QUERIES
// ==========================================
...
export async function getCalendarEvents(barbershopId = 'demo-shop-001', startDate, endDate) {
  ...
  try {
    ...
    // Transform database appointments to FullCalendar format
...
export async function getBarberResources(barbershopId = 'demo-shop-001') {
  ...
  try {
    // Query real barbers from database
    ...
    if (!barbers || barbers.length === 0) {
      // Return empty array instead of default mock resources
      console.log('📝 No barbers found in database')
      return []
    }
    
    // Transform database barbers to FullCalendar resource format
...
export async function getRecurringAppointments(barbershopId = 'demo-shop-001') {
  ...
  try {
    // Query recurring appointment templates
    ...
    // Transform to FullCalendar recurring event format
...
function parseAvailabilitySchedule(schedule) {
  // Parse JSON availability schedule to FullCalendar business hours format
  if (!schedule) {
...
// ==========================================
// SEED CALENDAR DATA (if tables are empty)
// ==========================================
