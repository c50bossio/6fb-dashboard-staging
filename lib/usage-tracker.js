  ...
  static async trackUsage(userId, eventType, quantity, metadata = {}) {
    ...
    try {
      ...
      const { data: usageEvent, error: insertError } = await supabase
        ...
        .insert({
          user_id: userId,
          barberbarbershop_id: null, // We'll set this later when we have barbershop context
  ...
  static async getCurrentUsage(userId) {
    try {
      ...
      if (usageError) {
        ...
        const realUsage = { ai_tokens: 0, sms_sent: 0, email_sent: 0 }
        const realCosts = { ai: 0, sms: 0, email: 0 }
      } else {
