...
export const edgeConfig = {
  ...
  config: {
    ...
    isFeatureEnabled: async (featureName) => {
      // Direct call to getValue to avoid circular dependency
      const flags = await edgeConfig.getValue('featureFlags', {
        aiInsightsV2: false,
        advancedCalendar: false,
        realTimeChat: false,
        premiumAnalytics: false,
  ...
  utils: {
    ...
    isMaintenanceActive: async (clientIP) => {
      // Direct call to getValue to avoid circular dependency
      const maintenance = await edgeConfig.getValue('maintenanceMode', {
        enabled: false,
        message: 'We are currently performing maintenance. Please check back soon.',
        allowedIPs: [],
      })
    ...
    getExperimentVariant: async (experimentName, userId) => {
      // Direct call to getValue to avoid circular dependency
      const experiments = await edgeConfig.getValue('experiments', {
        newDashboardDesign: {
          enabled: true,
          variants: ['control', 'redesign'],
          traffic: { control: 50, redesign: 50 },
