...
export async function getBusinessMetrics(barbershopId = DEMO_BARBERSHOP_ID, timeRange = '30d') {
  ...
  try {
    ...
    if (!data || data.length === 0) {
      // If business_metrics table is empty, calculate metrics from actual Supabase data
      // This ensures both dashboards show the same data
      try {
        console.log('📊 business_metrics table empty, calculating from real barbershop data...')
        ...
        // Get barbershop data (we know this exists)
        ...
        // Return calculated metrics from real Supabase data
        return {
...
export async function getAIInsights(barbershopId = 'DEMO_BARBERSHOP_ID', limit = 10) {
  const supabase = createClient()
...
export async function getAIAgents(barbershopId = 'DEMO_BARBERSHOP_ID') {
  const supabase = createClient()
...
export async function getBusinessRecommendations(barbershopId = 'DEMO_BARBERSHOP_ID', limit = 5) {
  const supabase = createClient()
...
export async function getAnalyticsData(barbershopId = 'DEMO_BARBERSHOP_ID') {
  const supabase = createClient()
...
export async function getPredictiveData(barbershopId = 'DEMO_BARBERSHOP_ID') {
  const supabase = createClient()
...
export async function getRealtimeMetrics(barbershopId = 'DEMO_BARBERSHOP_ID') {
  const supabase = createClient()
...
export async function getDashboardModeData(mode, barbershopId = 'DEMO_BARBERSHOP_ID') {
  try {
    switch (mode) {
      case 'executive':
