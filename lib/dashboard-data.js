...
export async function getBusinessMetrics(barbershopId, timeRange = '30d') {
  const supabase = createClient()
  
  // Validate barbershopId is provided
  if (!barbershopId) {
    throw new Error('barbershopId is required for business metrics')
  }
  ...
  try {
    ...
    if (!data || data.length === 0) {
      try {
        ...
        const shopConfig = await configService.getShopConfig(barbershopId)
...
export async function getAIInsights(barbershopId, limit = 10) {
  if (!barbershopId) {
    throw new Error('barbershopId is required for AI insights')
  }
  const supabase = createClient()
...
export async function getAIAgents(barbershopId) {
  if (!barbershopId) {
    throw new Error('barbershopId is required for AI agents')
  }
  const supabase = createClient()
...
export async function getBusinessRecommendations(barbershopId, limit = 5) {
  if (!barbershopId) {
    throw new Error('barbershopId is required for business recommendations')
  }
  const supabase = createClient()
...
export async function getAnalyticsData(barbershopId) {
  if (!barbershopId) {
    throw new Error('barbershopId is required for analytics data')
  }
  const supabase = createClient()
...
export async function getPredictiveData(barbershopId) {
  if (!barbershopId) {
    throw new Error('barbershopId is required for predictive data')
  }
  const supabase = createClient()
...
export async function getRealtimeMetrics(barbershopId) {
  if (!barbershopId) {
    throw new Error('barbershopId is required for realtime metrics')
  }
  const supabase = createClient()
...
export async function getDashboardModeData(mode, barbershopId) {
  if (!barbershopId) {
    throw new Error('barbershopId is required for dashboard mode data')
  }
  try {
    switch (mode) {
      case 'executive':
