  ...
  try {
    ...
    try {
      ...
      const { data, error: appointmentsError } = await supabase
        .from('appointments')
        .select('id, date, scheduled_at, status, price, service_id, customer_id, created_at, barber_id')
    ...
    // Filter appointments by time periods (using date/scheduled_at field)
    const allAppointments = appointments.filter(apt => new Date(apt.date || apt.scheduled_at) >= startDate)
    const currentMonthAppointments = allAppointments.filter(apt => 
      new Date(apt.date || apt.scheduled_at) >= currentMonthStart
    )
    const lastMonthAppointments = allAppointments.filter(apt => 
      new Date(apt.date || apt.scheduled_at) >= lastMonthStart && new Date(apt.date || apt.scheduled_at) <= lastMonthEnd
    )
