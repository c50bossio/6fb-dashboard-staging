...
export class SubscriptionService {
   ...
   * @param {string} params.barberbarbershopId - Barbershop ID (for business subscriptions)
   * @param {boolean} params.includeUsage - Include usage data
   * @returns {Promise<Object|null>} Subscription data or null
   */
  async getSubscription({ userId = null, barberbarbershopId = null, includeUsage = false }) {
    try {
      if (!userId && !barberbarbershopId) {
        throw new Error('Either userId or barberbarbershopId must be provided')
      }

      const cacheKey = `subscription-${userId || barberbarbershopId}-${includeUsage}`
      ...
      if (userId) {
        ...
        query = query.eq('barberbarbershop_id', barberbarbershopId)
      }
  ...
  async createSubscription(subscriptionData) {
    try {
      ...
      this.clearCache(userId, barberbarbershopId)
...
return subscription
   ...
   */
  clearCache(userId = null, barberbarbershopId = null) {
    const identifier = userId || barberbarbershopId
    if (identifier) {
