...
export class StaffService {
   ...
   */
  async getUserBarberbarbershopId() {
    try {
      ...
      const { data: profile } = await this.supabase
        .from('profiles')
        .select('barbershop_id, barberbarbershop_id, id')
        .eq('id', user.id)
        .single()
   ...
   */
  async loadStaffData(barberbarbershopId = null) {
    try {
      // Get barbershop ID if not provided
      if (!barberbarbershopId) {
        const { barbershopId } = await this.getUserBarberbarbershopId()
  ...
  async createStaffInvitation(invitationData) {
    try {
      const { barbershopId, userId } = await this.getUserBarberbarbershopId()
