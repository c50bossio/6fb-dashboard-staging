...
export class StaffService {
  ...
  async getUserBarbershopId() {
    try {
      ...
      const { data: profile } = await this.supabase
        .from('profiles')
        .select('shop_id, barbershop_id, id')
        .eq('id', user.id)
        .single()
      ...
      return { shopId, userId: user.id, profile }
    } catch (error) {
  ...
  async loadStaffData(barbershopId = null) {
    try {
      ...
      if (!barbershopId) {
        const { shopId } = await this.getUserBarbershopId()
        barbershopId = shopId
      }
  ...
  async createStaffInvitation(invitationData) {
    try {
      const { shopId, userId } = await this.getUserBarbershopId()
