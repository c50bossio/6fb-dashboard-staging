...
class UnifiedStaffService {
  ...
  async getStaff(barbershopId = null, options = {}) {
    ...
    try {
      ...
      try {
        console.log('🔐 Attempting authenticated staff endpoint...')
        ...
        if (authResponse.ok) {
          ...
          if (authData.success && authData.staff && authData.staff.length > 0) {
            ...
            console.log(`✅ Authenticated endpoint: Found ${authData.staff.length} staff members`)
          }
        } else if (authResponse.status === 401) {
          console.log('🚫 Authenticated endpoint returned 401 (expected for public contexts)')
        }
      } catch (authError) {
        console.log('⚠️ Authenticated endpoint failed:', authError.message)
      }

      // Step 2: If authenticated failed and we have barbershopId, try public endpoint
      if (!staffData && barbershopId) {
        try {
          console.log(`🌐 Attempting public endpoint for barbershop ${barbershopId}...`)
          ...
          if (publicResponse.ok) {
            ...
            if (publicData.success && publicData.staff && publicData.staff.length > 0) {
              ...
              console.log(`✅ Public endpoint: Found ${publicData.staff.length} staff members`)
            }
          } else {
            console.log(`❌ Public endpoint failed with status ${publicResponse.status}`)
          }
        } catch (publicError) {
          console.log('⚠️ Public endpoint failed:', publicError.message)
        }
      }
      ...
      if (!staffData && !barbershopId) {
        ...
        if (urlBarbershopId) {
          console.log(`🔍 Found barbershop ID in context: ${urlBarbershopId}`)
      ...
      console.log('❌ No staff data found from any endpoint')
      return {
        staff: [],
  ...
  invalidateCache(barbershopId = null) {
    if (barbershopId) {
      ...
      console.log(`🗑️ Cache invalidated for barbershop ${barbershopId}`)
    } else {
      // Clear all cache
      this.cache.clear()
      console.log('🗑️ All staff cache cleared')
    }
  }
