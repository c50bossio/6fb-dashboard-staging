...
class UnifiedStaffService {
  ...
  async getStaff(barbershopId = null, options = {}) {
    ...
    console.log('🚀 UnifiedStaffService.getStaff called with:', {
      barbershopId,
      options
    })
    ...
    try {
      ...
      try {
        console.log('🔐 UnifiedStaffService: Trying authenticated endpoint /api/staff')
        const authResponse = await fetch('/api/staff')
        console.log('🔐 UnifiedStaffService: Auth response status:', authResponse.status)
        
        if (authResponse.ok) {
          const authData = await authResponse.json()
          console.log('🔐 UnifiedStaffService: Auth data received:', {
            success: authData.success,
            staffCount: authData.staff?.length,
            barbershopId: authData.barbershop_id
          })
          if (authData.success && authData.staff && authData.staff.length > 0) {
            ...
            console.log('✅ UnifiedStaffService: Using authenticated data')
          }
        } else if (authResponse.status === 401) {
          console.log('⚠️ UnifiedStaffService: Auth failed (401), will try public endpoint')
        }
      } catch (authError) {
        console.log('⚠️ UnifiedStaffService: Auth endpoint error, will try public:', authError.message)
      }

      // Step 2: If authenticated failed and we have barbershopId, try public endpoint
      if (!staffData && barbershopId) {
        try {
          console.log('🔍 UnifiedStaffService: Trying public endpoint for barbershopId:', barbershopId)
          const publicResponse = await fetch(`/api/public/barbershop/${barbershopId}/barbers`)
          console.log('🔍 UnifiedStaffService: Public response status:', publicResponse.status)
          
          if (publicResponse.ok) {
            const publicData = await publicResponse.json()
            console.log('🔍 UnifiedStaffService: Public data received:', {
              success: publicData.success,
              staffCount: publicData.staff?.length,
              barbershopId: publicData.barbershop_id,
              barbershopName: publicData.barbershop_name
            })
            if (publicData.success && publicData.staff && publicData.staff.length > 0) {
              ...
              console.log('✅ UnifiedStaffService: Staff data set from public endpoint')
            } else {
              console.log('⚠️ UnifiedStaffService: Public endpoint returned no staff')
            }
          } else {
            console.log('❌ UnifiedStaffService: Public endpoint failed with status:', publicResponse.status)
          }
        } catch (publicError) {
      ...
      if (staffData) {
        console.log('🎯 UnifiedStaffService: Processing staff data with unified schema for', staffData.staff.length, 'staff members')
        ...
        console.log('✅ UnifiedStaffService: Returning unified schema staff:', {
          count: result.count,
          source: result.source,
          barbershopId: result.barbershop_id,
      ...
      console.log('⚠️ UnifiedStaffService: No staff data found, returning empty result for barbershopId:', barbershopId)
      return {
        staff: [],
  ...
  async getStaffUnified(barbershopId, options = {}) {
    ...
    console.log('🔄 UnifiedStaffService.getStaffUnified called with unified schema approach')
    ...
    try {
      ...
      if (useCache && this.cache.has(cacheKey)) {
        const cached = this.cache.get(cacheKey)
        if (Date.now() - cached.timestamp < this.cacheTimeout) {
          console.log('✅ UnifiedStaffService: Returning cached unified data')
          return cached.data
        }
      }
      ...
      console.log(`✅ UnifiedStaffService: Retrieved ${staffRecords.length} staff via unified query`)
      ...
      console.log('✅ UnifiedStaffService: Returning unified schema result:', {
        count: result.count,
        barbershopId: result.barbershop_id,
        source: result.source
      })
