...
class UnifiedStaffService {
  ...
  async getStaff(barbershopId = null, options = {}) {
    ...
    if (process.env.NODE_ENV === 'development') {
      console.log(`🏁 UnifiedStaffService: Starting getStaff for barbershop ${barbershopId}`)
    }
    ...
    try {
      ...
      if (useCache && !forceRefresh && this.cache.has(cacheKey)) {
        ...
        if (Date.now() - cached.timestamp < this.cacheTimeout) {
          if (process.env.NODE_ENV === 'development') {
            console.log(`📦 UnifiedStaffService: Using cached data for ${barbershopId}`)
          }
          return cached.data
        }
      }
      ...
      // Step 1: Try authenticated endpoint first with limited retry logic
      try {
        if (process.env.NODE_ENV === 'development') {
        ...
        try {
          // Build URL with barbershop_id parameter if provided
          const staffUrl = barbershopId
            ? `/api/staff?barbershop_id=${encodeURIComponent(barbershopId)}`
            : '/api/staff'

          let authResponse = await fetch(staffUrl, {
            credentials: 'include',
            headers: {
          ...
          // Only retry once on 401 if retryAuth is enabled (reduced from multiple retries)
          if (authResponse.status === 401 && retryAuth) {
            if (process.env.NODE_ENV === 'development') {
              console.log('Auth endpoint returned 401, waiting 800ms before single retry...')
            }
            ...
            try {
              // Build URL with barbershop_id parameter if provided (retry)
              const retryStaffUrl = barbershopId
                ? `/api/staff?barbershop_id=${encodeURIComponent(barbershopId)}`
                : '/api/staff'

              authResponse = await fetch(retryStaffUrl, {
                credentials: 'include',
                headers: {
                  'Content-Type': 'application/json',
                },
              ...
              if (retryError.name === 'AbortError') {
                console.warn('Auth endpoint retry timed out after 10s')
              } else {
                throw retryError
              }
            }
          ...
          if (authResponse.ok) {
            ...
            console.warn(`Auth endpoint returned ${authResponse.status}, will try public endpoint`)
          }
        } catch (fetchError) {
          clearTimeout(timeoutId)
          if (fetchError.name === 'AbortError') {
            console.warn('Auth endpoint timed out after 10s, will try public endpoint')
          } else {
            throw fetchError
          }
        }
      ...
      if (!staffData && barbershopId) {
        try {
          if (process.env.NODE_ENV === 'development') {
            console.log(`🌐 UnifiedStaffService: Trying public endpoint for barbershop ${barbershopId}`)
          }
          ...
          try {
            ...
            } else if (publicResponse.status === 503) {
              // Service Unavailable - likely environment variable issue
              const errorData = await publicResponse.json().catch(() => ({}))
              console.error('❌ UnifiedStaffService: Public API service unavailable (503)')
              if (errorData.troubleshooting) {
                console.error('   Troubleshooting steps:', errorData.troubleshooting)
              }
            } else {
              if (process.env.NODE_ENV === 'development') {
            ...
            clearTimeout(timeoutId)
            if (fetchError.name === 'AbortError') {
              console.warn('Public endpoint timed out after 10s')
            } else {
              throw fetchError
            }
          }
        } catch (publicError) {
          console.error('❌ UnifiedStaffService: Error calling public endpoint:', publicError.message)
        }
      }

      // Step 3: If we still don't have barbershopId, try to get it from URL or context
      if (!staffData && !barbershopId) {
        const urlBarberbarbershopId = this.extractBarberbarbershopIdFromContext()
        if (urlBarberbarbershopId) {
          
          return this.getStaff(urlBarberbarbershopId, { ...options, useCache: false })
        }
      }
   ...
   */
  extractBarberbarbershopIdFromContext() {
  ...
  async getStaffUnified(barbershopId, options = {}) {
    ...
    if (process.env.NODE_ENV === 'development') {
      console.log(`🔍 UnifiedStaffService.getStaffUnified: Starting query for barbershop ${barbershopId}`)
    }
    ...
    try {
      ...
      if (useCache && this.cache.has(cacheKey)) {
        ...
        if (Date.now() - cached.timestamp < this.cacheTimeout) {
          if (process.env.NODE_ENV === 'development') {
            console.log(`📦 UnifiedStaffService.getStaffUnified: Using cached data`)
          }
          return cached.data
        }
      }

      // Query unified staff table directly - no joins, no complex logic
      ...
      if (staffError) {
        console.error('❌ UnifiedStaffService.getStaffUnified: Staff query failed:', staffError)
        throw new Error(`Staff query failed: ${staffError.message}`)
      }

      if (process.env.NODE_ENV === 'development') {
        console.log(`✅ UnifiedStaffService.getStaffUnified: Found ${staffRecords?.length || 0} staff members`)
      }
      ...
      // Enhance with additional data if requested
      const enhancedStaff = normalizedStaff
      ...
      if (process.env.NODE_ENV === 'development') {
        console.log(`✅ UnifiedStaffService.getStaffUnified: Returning ${result.count} staff members from unified staff table`)
      }
