...
class UnifiedStaffService {
        ...
        if (authResponse.ok) {
          ...
          if (authData.success && authData.staff && authData.staff.length > 0) {
            staffData = {
              ...
              barberbarbershop_id: authData.barberbarbershop_id,
          ...
          if (publicResponse.ok) {
            ...
            if (publicData.success && publicData.staff && publicData.staff.length > 0) {
              staffData = {
                ...
                barberbarbershop_id: publicData.barberbarbershop_id,
   ...
   * FIXED: Single source of truth - only check barberbarbershop_id
   */
  extractBarberbarbershopIdFromContext() {
    if (typeof window === 'undefined') return null

    const urlParams = new URLSearchParams(window.location.search)
    // CONSOLIDATED: Only use barberbarbershop_id as single field name
    return urlParams.get('barberbarbershop_id') || 
    ...
    try {
      ...
      const normalizedStaff = staff.map(member => {
        ...
        return {
          ...
          barberbarbershop_id: member.barberbarbershop_id || barbershopId,
