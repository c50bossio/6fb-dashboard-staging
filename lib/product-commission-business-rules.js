...
export function validateProductSaleData(saleData) {
  ...
  // Required fields
  const requiredFields = ['product_sale_id', 'barbershop_id', 'barber_id', 'line_items', 'total_amount']
...
export function validateProductReturnData(returnData) {
  ...
  // Required fields
  const requiredFields = ['original_product_sale_id', 'barbershop_id', 'barber_id', 'returned_items']
  for (const field of requiredFields) {
    if (!returnData[field]) {
      errors.push(`${field} is required`)
    }
