...
class Cin7SyncService {
   ...
   */
  async syncInventory(barberbarbershopId, options = {}) {
    const {
      forceFullSync = false,
      syncStockOnly = false,
...
try {
      ...
      const connection = await this.getConnection(supabase, barberbarbershopId)
      if (!connection) {
      ...
      try {
        if (!syncStockOnly) {
          // Sync products first
          const productResults = await this.syncProducts(cin7Client, supabase, barberbarbershopId, forceFullSync)
          results = { ...results, ...productResults }
        }

        // Sync stock levels
        const stockResults = await this.syncStockLevels(cin7Client, supabase, barberbarbershopId)
...
return { success: true, results }
   ...
   */
  async syncProducts(cin7Client, supabase, barberbarbershopId, forceFullSync = false) {
    // // Debug log removed for production
const results = {
      productsProcessed: 0,
      productsCreated: 0,
...
// Process products in batches
      for (let i = 0; i < allProducts.length; i += this.batchSize) {
        ...
        try {
          const batchResults = await this.processBatch(batch, supabase, barberbarbershopId, forceFullSync)
   ...
   */
  async processBatch(products, supabase, barberbarbershopId, forceFullSync) {
    const results = {
      processed: 0,
      created: 0,
      updated: 0,
    ...
    for (const product of products) {
      try {
        ...
        const inventoryItem = await this.mapCin7ProductToInventory(product, barberbarbershopId)
   ...
   */
  async syncStockLevels(cin7Client, supabase, barberbarbershopId) {
    // // Debug log removed for production
const results = {
      stockUpdated: 0,
      errors: []
...
const result = await this.executeWithRetry(
          ...
          if (result && result.products && result.products.length > 0) {
            ...
            console.log(`Fetched ${result.products.length} products from page ${page}`)
          ...
          await this.delay(this.rateLimitDelay)
              ...
              console.log(`Retrieved ${allProducts.length} products from CIN7`)
      return allProducts
...
await this.delay(60000) // Wait 1 minute for rate limit
   ...
   */
  async mapCin7ProductToInventory(product, barberbarbershopId) {
    try {
      return {
  ...
  async getConnection(supabase, barberbarbershopId) {
    try {
  ...
  async queueSync(barberbarbershopId, operation) {
