...
class AutomationQueueManager {
   ...
   */
  static async addTask(barberbarbershopId, taskData) {
    ...
    const dedupeKey = `${barberbarbershopId}:${task_type}:${client_id || 'null'}:${appointment_id || 'null'}`;
    ...
    await QueryCache.invalidate(`queue:status:${barberbarbershopId}`);
   ...
   */
  static async addTasksBatch(barberbarbershopId, tasks) {
   ...
   */
  static async getQueueStatus(barberbarbershopId) {
    const cacheKey = `queue:status:${barberbarbershopId}`;
...
class AutomationLogger {
   ...
   */
  static async logStart(barberbarbershopId, automationType, context = {}) {
    const { data, error } = await supabase
   ...
   */
  static async getPerformanceMetrics(barberbarbershopId, timeRange = '24 hours') {
    const cacheKey = `metrics:${barberbarbershopId}:${timeRange}`;
...
class PaymentProcessor {
  ...
  static async logPaymentAttempt(paymentData) {
    ...
    try {
      ...
      const { data, error } = await client
        ...
        .insert({
          barberbarbershop_id: paymentData.barberbarbershop_id,
      ...
      await QueryCache.invalidate(`payments:${paymentData.barberbarbershop_id}`);
   ...
   */
  static async getPaymentAnalytics(barberbarbershopId, dateRange = '30 days') {
    const cacheKey = `payments:${barberbarbershopId}:${dateRange}`;
...
class RiskScoreManager {
  ...
  static async storeRiskScore(scoreData) {
    const { data, error } = await supabase
      .from('risk_scores')
      .insert({
        barberbarbershop_id: scoreData.barberbarbershop_id,
    ...
    await QueryCache.invalidate(`risk:${scoreData.barberbarbershop_id}`);
   ...
   */
  static async getHighRiskClients(barberbarbershopId, riskType, threshold = 0.7, limit = 50) {
    const cacheKey = `risk:high:${barberbarbershopId}:${riskType}:${threshold}`;
...
class ReminderManager {
  ...
  static async scheduleReminder(reminderData) {
    const dedupeKey = `${reminderData.barberbarbershop_id}:${reminderData.appointment_id}:${reminderData.reminder_type}`;
    ...
    const { data, error } = await supabase
      ...
      .select(`
        *,
        ...
        barbershops:barberbarbershop_id (name, phone)
      `)
      .eq('status', 'PENDING')
...
BEGIN
  ...
  RETURNING 
    automation_queue.id,
    automation_queue.barberbarbershop_id,
...
BEGIN
  ...
  GROUP BY barberbarbershop_id, automation_type
  ON CONFLICT (barberbarbershop_id, metric_date, automation_type) WHERE metric_hour IS NULL
  DO UPDATE SET
