...
class ConfigurationService {
   ...
   */
  async getShopConfig(barbershopId) {
    // Check cache first
    const cacheKey = `shop_config_${barbershopId}`
    ...
    try {
      ...
      if (error || !shop) {
        console.warn(`Shop config not found for ID ${barbershopId}, using defaults`)
   ...
   */
  async getServicesConfig(barbershopId) {
    try {
      const supabase = createClient()
   ...
   */
  clearCache(barbershopId = null) {
    if (barbershopId) {
      // Clear specific shop cache
      const keys = Array.from(this.cache.keys()).filter(key => key.includes(barbershopId))
  ...
  getApiEndpoints() {
    ...
    return {
      // Backend APIs (now served by Next.js)
      backend: {
        base: baseUrl,
