...
class Cin7Client {
  constructor(accountId, apiKey) {
    ...
    this.apiKey = apiKey
    // Use working URL structure from API logs
    this.baseUrl = 'https://inventory.dearsystems.com/externalapi'
    this.accountUrl = 'https://inventory.dearsystems.com/ExternalApi'
  }
  ...
  async testConnection() {
    try {
      // Use the exact working URL from API logs
      const url = `${this.accountUrl}/Me`
  ...
  async getProducts(page = 1, limit = 100) {
    try {
      ...
      console.log(`Fetching products from CIN7: page=${page}, limit=${limit}`)
      const result = await this.makeRequest(`/products?${params}`)
      console.log('CIN7 API Response:', {
        hasProductList: !!result.ProductList,
        productCount: result.ProductList?.length,
        total: result.Total,
        responseKeys: Object.keys(result)
      })
      
      // CIN7 API v2 returns products in 'ProductList' field
      const products = result.ProductList || result.Products || []
      ...
      try {
        console.log('Trying without pagination parameters...')
        ...
        console.error('Alternate attempt also failed:', altError.message)
        throw error
      }
    }
  }
