...
class UnifiedStripeManager {
  ...
  async orchestrateSetup(barbershopId, setupConfig = {}) {
    ...
    try {
      ...
      // Step 2: Check onboarding completion with enhanced validation
      const needsOnboarding = !currentStatus.connect_account.onboarding_completed || 
                             !currentStatus.connect_account.charges_enabled || 
                             !currentStatus.connect_account.payouts_enabled ||
                             !currentStatus.connect_account.details_submitted

      if (needsOnboarding) {
        console.log('🔧 [Stripe Setup] Account needs onboarding completion:', {
          onboarding_completed: currentStatus.connect_account.onboarding_completed,
          charges_enabled: currentStatus.connect_account.charges_enabled,
          payouts_enabled: currentStatus.connect_account.payouts_enabled,
          details_submitted: currentStatus.connect_account.details_submitted
        })

        const onboardingResult = await this.generateOnboardingLink(
          currentStatus.connect_account.account_id,
          returnUrl || `${typeof window !== 'undefined' ? window.location.origin : ''}/shop/settings/payment-setup?onboarding=complete`
        )
        ...
        console.log('✅ [Stripe Setup] Generated onboarding URL:', onboardingResult.url)
        return orchestrationResult
      }
