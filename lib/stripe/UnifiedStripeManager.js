...
class UnifiedStripeManager {
   ...
   */
  async getUnifiedStatus(barberbarbershopId, forceRefresh = false) {
    const cacheKey = `status_${barberbarbershopId}`
    ...
    try {
      ...
      // Notify subscribers
      this.notifySubscribers('status_updated', { barberbarbershopId, status })
   ...
   */
  async orchestrateSetup(barberbarbershopId, setupConfig = {}) {
    const {
      email,
    ...
    try {
      // Get current status
      const currentStatus = await this.getUnifiedStatus(barberbarbershopId, true)
      ...
      if (!currentStatus.connect_account.exists) {
        const createResult = await this.createConnectAccount(barberbarbershopId, {
          email,
          businessType,
          returnUrl
        })
      ...
      if (!currentStatus.connect_account.onboarding_completed) {
        const onboardingResult = await this.generateOnboardingLink(
      ...
      if (enableTerminal && 
        ...
        const terminalResult = await this.configureTerminal(barberbarbershopId, {
      ...
      if (enableSplitPayments && 
        ...
        const splitResult = await this.configureSplitPayments(barberbarbershopId, {
          account_id: currentStatus.connect_account.account_id
        })
      ...
      // Update final status
      orchestrationResult.current_status = await this.getUnifiedStatus(barberbarbershopId, true)
   ...
   */
  async createConnectAccount(barberbarbershopId, accountConfig) {
    try {
      ...
      // Invalidate cache
      this.cache.delete(`status_${barberbarbershopId}`)
   ...
   */
  async configureTerminal(barberbarbershopId, config) {
    try {
   ...
   */
  async configureSplitPayments(barberbarbershopId, config) {
    try {
   ...
   */
  async performHealthCheck(barberbarbershopId) {
    try {
      const response = await fetch(`/api/stripe/unified/health?barberbarbershop_id=${barberbarbershopId}`)
      const result = await response.json()
      ...
      this.cache.set(`status_${barberbarbershopId}`, {
        data: result.status,
        timestamp: Date.now()
      })
   ...
   */
  invalidateCache(barberbarbershopId) {
    this.cache.delete(`status_${barberbarbershopId}`)
  }
   ...
   */
  async isSetupComplete(barberbarbershopId) {
    const status = await this.getUnifiedStatus(barberbarbershopId)
   ...
   */
  async getSetupProgress(barberbarbershopId) {
    const status = await this.getUnifiedStatus(barberbarbershopId)
   ...
   */
  async getStatusMessage(barberbarbershopId) {
    const status = await this.getUnifiedStatus(barberbarbershopId)
   ...
   */
  async hasCapability(barberbarbershopId, capability) {
    const status = await this.getUnifiedStatus(barberbarbershopId)
   ...
   */
  async migrateExistingSetup(barberbarbershopId) {
    const supabase = this.getSupabase()
    if (!supabase) return { error: 'Client not initialized' }
    ...
    try {
      ...
      const status = await this.getUnifiedStatus(barberbarbershopId, true)
