...
export async function ensureUserShop(profile) {
  ...
  // Check barberbarbershop_id as fallback
...
export async function getOrAssignShopId(user, profile) {
  ...
  if (profile?.shop_id) {
    console.log('[Shop Resolution] Using profile.shop_id:', profile.shop_id)
    return profile.shop_id
  }
  ...
  if (user?.barberbarbershop_id) {
    console.log('[Shop Resolution] Using user.barberbarbershop_id:', user.barberbarbershop_id)
    return user.barberbarbershop_id
  }
    ...
    try {
      ...
      if (!ownerError && ownedShops && ownedShops.length > 0) {
        ...
        await supabase
          .from('profiles')
          .update({ 
            barberbarbershop_id: ownedShops[0].id,
            updated_at: new Date().toISOString()
          })
          .eq('id', userId)
      ...
      if (!staffError && staffRecord && staffRecord.barberbarbershop_id) {
        console.log('[Shop Resolution] Found shop ID via staff association:', staffRecord.barberbarbershop_id)
        // Update profile with this barberbarbershop_id for future use to avoid repeated lookups
        await supabase
          .from('profiles')
          .update({ 
          ...
          .eq('id', userId)
        return staffRecord.barberbarbershop_id
      }
