...
class AnalyticsCache {
  ...
  invalidate(typeOrPattern) {
    ...
    if (invalidated > 0) {
      console.log(`🗑️ Cache invalidated ${invalidated} entries for pattern: ${typeOrPattern}`)
    }
  ...
  evictOldest() {
    ...
    if (oldestKey) {
      ...
      console.log(`♻️ Cache evicted oldest entry: ${oldestKey.substring(0, 50)}...`)
    }
  }
...
export async function cacheQuery(type, params, queryFunction) {
  try {
    ...
    console.log(`⚡ Query executed for ${type} in ${queryTime}ms`)
