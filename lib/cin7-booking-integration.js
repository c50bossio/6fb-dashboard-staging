...
class Cin7BookingIntegration {
   ...
   */
  async checkServiceAvailability(serviceId, appointmentDate, barbershopId) {
    console.log(`🔍 Checking inventory availability for service ${serviceId}`)
   ...
   */
  async calculateAvailableStock(productId, appointmentDate, barbershopId) {
    const supabase = createClient()
   ...
   */
  async suggestAlternativeServices(serviceId, barbershopId, insufficientProducts) {
    const supabase = createClient()
  ...
  async reserveInventoryForAppointment(appointmentId) {
    console.log(`🔒 Reserving inventory for appointment ${appointmentId}`)
    ...
    try {
      ...
      if (appointment.status !== 'confirmed') {
        console.log('⚠️ Appointment not confirmed, skipping inventory reservation')
      ...
      for (const service of appointment.services) {
        for (const serviceProduct of service.service_products) {
          ...
          console.log(`✅ Reserved ${requiredQty}x ${product.name} for appointment ${appointmentId}`)
        }
      }
  ...
  async releaseInventoryReservation(appointmentId, consumeInventory = false) {
    console.log(`🔓 Releasing inventory for appointment ${appointmentId} (consume: ${consumeInventory})`)
    ...
    try {
      ...
      for (const reservation of reservations) {
        ...
        if (consumeInventory) {
          ...
          console.log(`✅ Consumed ${reservedQty}x ${product.name} for completed appointment`)
        } else {
          ...
          console.log(`✅ Released ${reservedQty}x ${product.name} from cancelled appointment`)
        }
   ...
   */
  async updateServiceAvailability(barbershopId) {
    console.log(`🔄 Updating service availability for barbershop ${barbershopId}`)
      ...
      for (const service of services) {
        ...
        if (serviceAvailable !== service.is_available) {
          ...
          if (updateError) {
            ...
            console.log(`✅ ${service.name}: ${serviceAvailable ? 'Available' : 'Unavailable'}`)
          }
        }
      }
   ...
   */
  async generateServiceImpactAlerts(barbershopId) {
    const supabase = createClient()
