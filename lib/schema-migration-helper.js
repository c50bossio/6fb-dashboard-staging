...
export async function getUnifiedBarbershopId(profile, userId = null) {
  try {
    ...
    if (profile.barbershop_id) {
      console.log('✅ Found barbershop_id (unified schema):', profile.barbershop_id)
    ...
    if (userId) {
      console.log('🔍 No direct barbershop association, checking staff relationships...')
...
async function getStaffBarbershopId(userId) {
  try {
    ...
    if (barbershopId) {
      console.log('✅ Found barbershop via staff relationship:', barbershopId)
    } else {
      console.log('ℹ️ User is not an active staff member at any barbershop')
    }
...
async function migrateLegacyShopIdToUnified(userId, shopId) {
  try {
    ...
    console.log(`✅ Migrated legacy shop_id to barbershop_id for user ${userId}`)
    return true
...
export async function queryUnifiedStaffData(supabase, barbershopId, options = {}) {
  ...
  try {
    ...
    if (!staffRecords || staffRecords.length === 0) {
      console.log('queryUnifiedStaffData: No staff found for barbershop:', barbershopId)
      return []
    }
...
export async function cleanupForeignKeyReferences(dryRun = true) {
  ...
  try {
    ...
    if (conflictingProfiles && conflictingProfiles.length > 0) {
      console.log(`Found ${conflictingProfiles.length} profiles with conflicting shop references`)
      
      for (const profile of conflictingProfiles) {
        if (dryRun) {
          console.log(`[DRY RUN] Would resolve conflict for profile ${profile.id}: shop_id=${profile.shop_id} vs barbershop_id=${profile.barbershop_id}`)
        } else {
    ...
    if (legacyProfiles && legacyProfiles.length > 0) {
      console.log(`Found ${legacyProfiles.length} profiles with legacy shop_id only`)
      
      for (const profile of legacyProfiles) {
        if (dryRun) {
          console.log(`[DRY RUN] Would migrate profile ${profile.id}: shop_id=${profile.shop_id} -> barbershop_id=${profile.shop_id}`)
        } else {
    ...
    if (dryRun) {
      ...
      console.log('Set dryRun=false to apply changes')
    }
