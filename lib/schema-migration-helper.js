 ...
 */
export async function getUnifiedBarberbarbershopId(profile, userId = null) {
  try {
    if (!profile) {
      console.warn('getUnifiedBarberbarbershopId: No profile provided')
      return null
    }

    // PRIMARY: Check barberbarbershop_id field (unified schema)
    if (profile.barbershop_id) {
      return profile.barbershop_id
    }

    // FALLBACK: Legacy barbershop_id during migration period
    if (profile.shop_id) {
      console.warn('⚠️ Found legacy barbershop_id, should be migrated to barberbarbershop_id:', profile.shop_id)
...
return await getStaffBarberbarbershopId(userId)
    }

    console.warn('getUnifiedBarberbarbershopId: No barbershop association found for profile')
    return null

  } catch (error) {
    console.error('getUnifiedBarberbarbershopId: Error determining barbershop ID:', error)
    return null
  }
}
 ...
 */
async function getStaffBarberbarbershopId(userId) {
  try {
    ...
    if (error) {
      console.error('getStaffBarberbarbershopId: Database error:', error)
      return null
    }
    ...
    console.error('getStaffBarberbarbershopId: Unexpected error:', error)
    return null
  }
}

/**
 * Migrate legacy barbershop_id to unified barberbarbershop_id field
 * @param {string} userId - User ID to update
 * @param {string} barbershopId - Legacy barbershop_id value
 * @returns {Promise<boolean>} Success status
 */
async function migrateLegacyShopIdToUnified(userId, barbershopId) {
  try {
    const supabase = createClient()
    ...
    const { error } = await supabase
      ...
      .update({ 
        ...
        // barbershop_id: barbershopId  // Uncomment if you want to keep legacy field
      })
      .eq('id', userId)
...
export async function validateUserForeignKeys(userId) {
  ...
  try {
    ...
    const { data: profile, error: profileError } = await supabase
      .from('profiles')
      .select('id, barberbarbershop_id, barbershop_id, role')
      .eq('id', userId)
      .single()
    ...
    // Check barberbarbershop_id consistency
    if (profile.barbershop_id) {
      ...
      if (barbershopError) {
        results.isValid = false
        results.issues.push(`Invalid barberbarbershop_id reference: ${profile.barbershop_id}`)
      } else {
    ...
    if (profile.shop_id && profile.barbershop_id && profile.shop_id !== profile.barbershop_id) {
      results.isValid = false
      results.issues.push(`Conflicting shop references: barbershop_id=${profile.shop_id} vs barberbarbershop_id=${profile.barbershop_id}`)
      results.recommendations.push(`Migrate to single barberbarbershop_id field`)
    }
    ...
    if (!profile.barbershop_id && !profile.shop_id) {
      const { data: staffRecords } = await supabase
        .from('barbershop_staff')
        .select('barberbarbershop_id, is_active')
        .eq('user_id', userId)

      if (staffRecords && staffRecords.length > 0) {
        const activeStaffRecords = staffRecords.filter(r => r.is_active)
        if (activeStaffRecords.length === 1) {
          results.recommendations.push(`Consider setting barberbarbershop_id to ${activeStaffRecords[0].barberbarbershop_id} (from staff relationship)`)
...
export function normalizeStaffRecords(staffRecords) {
  ...
  return staffRecords.map(record => ({
    ...
    // UNIFIED: Always use barberbarbershop_id
    barberbarbershop_id: record.barberbarbershop_id,
 ...
 * @param {string} barberbarbershopId - Barbershop ID to query
 * @param {Object} options - Query options
 * @returns {Promise<Array>} Normalized staff records
 */
export async function queryUnifiedStaffData(supabase, barberbarbershopId, options = {}) {
  const { 
...
export async function cleanupForeignKeyReferences(dryRun = true) {
  ...
  try {
    ...
    const { data: conflictingProfiles, error: conflictError } = await supabase
      .from('profiles')
      .select('id, barbershop_id, barberbarbershop_id')
      .neq('barbershop_id', 'barberbarbershop_id')
      .not('barbershop_id', 'is', null)
      .not('barberbarbershop_id', 'is', null)
...
for (const profile of conflictingProfiles) {
        if (dryRun) {
          ...
          // In real mode, prefer barberbarbershop_id over barbershop_id
          const { error: updateError } = await supabase
            .from('profiles')
            .update({ barbershop_id: null }) // Clear legacy field
            .eq('id', profile.id)
    ...
    // Find profiles with only shop_id (need migration to barberbarbershop_id)
    const { data: legacyProfiles, error: legacyError } = await supabase
      .from('profiles')
      .select('id, barbershop_id')
      .not('barbershop_id', 'is', null)
      .is('barberbarbershop_id', null)
        ...
        if (dryRun) {
          ...
          const { error: migrateError } = await supabase
            ...
            .update({ 
              ...
              barbershop_id: null // Remove legacy field
            })
            .eq('id', profile.id)
