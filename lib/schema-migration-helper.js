...
export async function getUnifiedbarbershopId(profile, userId = null) {
  try {
    ...
    if (profile.barbershop_id) {
      console.warn('⚠️ Found legacy barbershop_id, should be migrated to barbershop_id:', profile.barbershop_id)
      
      // Optional: Auto-migrate legacy field
      if (userId) {
        await migrateLegacyShopIdToUnified(userId, profile.barbershop_id)
      }
...
/**
 * Migrate legacy barbershop_id to unified barbershop_id field
...
export async function validateUserForeignKeys(userId) {
  ...
  try {
    ...
    if (profile.barbershop_id && profile.barbershop_id && profile.barbershop_id !== profile.barbershop_id) {
      results.isValid = false
      results.issues.push(`Conflicting shop references: barbershop_id=${profile.barbershop_id} vs barbershop_id=${profile.barbershop_id}`)
...
export async function cleanupForeignKeyReferences(dryRun = true) {
  ...
  try {
    ...
    const { data: conflictingProfiles, error: conflictError } = await supabase
      ...
      .neq('barbershop_id', 'barbershop_id')
...
for (const profile of conflictingProfiles) {
        if (dryRun) {
          ...
          // In real mode, prefer barbershop_id over barbershop_id
