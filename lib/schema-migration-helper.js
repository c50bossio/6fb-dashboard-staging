...
export async function getUnifiedbarbershopId(profile, userId = null) {
  try {
    ...
    if (profile.shop_id) {
      console.warn('⚠️ Found legacy barbershop_id, should be migrated to barbershop_id:', profile.shop_id)
...
export async function validateUserForeignKeys(userId) {
  ...
  try {
    ...
    if (profile.shop_id && profile.barbershop_id && profile.shop_id !== profile.barbershop_id) {
      results.isValid = false
      results.issues.push(`Conflicting shop references: barbershop_id=${profile.shop_id} vs barbershop_id=${profile.barbershop_id}`)
