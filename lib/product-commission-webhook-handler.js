...
export async function handleProductSaleWebhook(productSaleData, supabase) {
  ...
  try {
    ...
    if (!product_sale_id || !barberbarbershop_id || !barber_id || !line_items || !total_amount) {
    ...
    const { data: arrangement } = await financialService.getArrangement(barber_id, barberbarbershop_id)
 ...
 * @param {Object} supabase - Supabase client instance
 */
async function updateBarberBalanceForProduct(barberId, barberbarbershopId, commissionAmount, transactionRecords, supabase) {
  try {
    // Get current balance
...
export async function handleProductSalePayment(paymentIntent, supabase) {
  try {
    ...
    const productSaleData = {
      ...
      barberbarbershop_id: productSale.barberbarbershop_id,
...
export async function updateProductInventoryAfterSale(saleData, supabase) {
  try {
    const { line_items, barberbarbershop_id, barber_id } = saleData
