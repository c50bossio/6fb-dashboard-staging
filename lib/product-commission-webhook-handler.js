...
export async function handleProductSaleWebhook(productSaleData, supabase) {
  ...
  try {
    ...
    console.log(`🛍️ Processing product sale: ${product_sale_id} for barber ${barber_id}`)
    ...
    if (!arrangement) {
      console.log(`ℹ️ No financial arrangement found for product sale: ${product_sale_id}`)
      return { 
        success: true, 
    ...
    if (!arrangement.product_commission_rate && 
      ...
      console.log(`ℹ️ Product commissions not configured for barber ${barber_id}`)
      return { 
        success: true, 
    ...
    console.log(`💰 Product commission calculated: $${commissionCalculation.barberAmount} (Base: $${commissionCalculation.baseCommissionAmount}, Tier Bonus: $${commissionCalculation.tierBonusAmount || 0})`)
    ...
    console.log(`✅ Product commission processed successfully in ${processingTime}ms`)

    // Log tier achievement if applicable
    if (commissionCalculation.tierInfo?.tierUpgrade) {
      console.log(`🎉 Product sale triggered tier upgrade! Barber ${barber_id} reached ${commissionCalculation.tierInfo.applicableTier.name} tier`)
    }
...
export async function handleProductReturnWebhook(returnData, supabase) {
  ...
  try {
    ...
    console.log(`🔄 Processing product return: ${original_product_sale_id} for barber ${barber_id}`)
    ...
    console.log(`💸 Product return processed: Commission adjustment: $${returnResult.data.total_commission_adjustment}, Tier adjustment: $${returnResult.data.total_tier_adjustment}`)
    ...
    const processingTime = Date.now() - startTime
    console.log(`✅ Product return processed successfully in ${processingTime}ms`)
...
async function updateBarberBalanceForProduct(barberId, barbershopId, commissionAmount, transactionRecords, supabase) {
  try {
    ...
    console.log(`💰 Balance updated for barber ${barberId}: +$${commissionAmount} (product commission)`)
  } catch (error) {
...
async function sendProductCommissionNotification(notificationData) {
  try {
    ...
    console.log(`📧 Product commission notification: $${commissionAmount} earned from ${products.length} product(s) for barber ${barberId}`)

    if (tierInfo?.tierUpgrade) {
      console.log(`🎉 Tier upgrade notification: Barber ${barberId} reached ${tierInfo.applicableTier.name} tier!`)
    }
...
async function sendProductReturnNotification(notificationData) {
  try {
    ...
    console.log(`📧 Product return notification: Commission adjusted by $${commissionAdjustment} for barber ${barberId}`)

    if (Math.abs(tierAdjustment) > 0) {
      console.log(`📊 Tier progress adjusted by $${tierAdjustment} due to product return`)
    }
...
export async function handleProductSalePayment(paymentIntent, supabase) {
  try {
    ...
    if (productSale.commission_calculated) {
      console.log(`ℹ️ Commission already calculated for product sale: ${productSaleId}`)
    ...
    console.log(`🛍️ Product sale commission processed with payment: ${result.success ? 'Success' : 'Failed'}`)
...
export async function updateProductInventoryAfterSale(saleData, supabase) {
  try {
    ...
    console.log(`📦 Inventory updated for ${line_items.length} products`)
  } catch (error) {
