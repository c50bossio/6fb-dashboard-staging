  ...
  rules: {
    'no-mock-data': {
      ...
      create(context) {
        ...
        return {
          // Check variable declarations for mock data
          VariableDeclarator(node) {
            const name = node.id.name;
            
            // Check for mock-related variable names
            ...
            // Check for hardcoded arrays with suspicious data
          ...
          // Check function declarations for mock generators
          ...
          // Check string literals for mock data patterns
          Literal(node) {
            if (typeof node.value === 'string') {
              const value = node.value;
              
              // Skip URLs, imports, and very short strings
              if (value.length < 3 || 
          ...
          // Check for calls to Math.random() in data generation context
          CallExpression(node) {
            if (node.callee.type === 'MemberExpression' &&
                node.callee.object.name === 'Math' &&
                node.callee.property.name === 'random') {
              
              // Check if this is being used to generate data
              const parent = node.parent;
              if (parent && parent.type === 'MemberExpression' ||
                  (parent && parent.type === 'BinaryExpression')) {
                // Check surrounding context for data generation patterns
    ...
    'no-hardcoded-ids': {
      ...
      create(context) {
        return {
          Literal(node) {
            if (typeof node.value === 'string') {
              // Check for UUID-like strings or typical ID patterns
              const value = node.value;
              const idPatterns = [
              ...
              // Check if this looks like a hardcoded ID
              if (idPatterns.some(pattern => pattern.test(value))) {
                // Check context - allow in certain cases
                const parent = node.parent;
                
                // Allow in object property keys
                if (parent.type === 'Property' && parent.key === node) {
                  return;
                }
                
                // Allow in test files or migrations
                const filename = context.getFilename();
