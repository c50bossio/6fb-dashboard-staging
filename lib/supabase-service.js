 ...
 * Last updated: 2025-08-29T13:07:00Z - Fixed total_price -> price column name
 * 
 * Last updated: 2025-08-29T12:52:00Z - Fixed column name mismatches
 */
...
class SupabaseService {
  ...
  async getServices(shopId, options = {}) {
    ...
    return executeQuery(async () => {
      ...
      let query = this.client
        .from('services')
        .select('*')
        .eq('barbershop_id', targetShopId) // Using barbershop_id as per Phase 1-2 standardization

      if (isActive !== null) {
        query = query.eq('active', isActive) // Using active column from migration
      }
      if (category) {
        query = query.eq('category', category)
      }

      // Order by name since display_order doesn't exist
  ...
  async createService(serviceData) {
    return executeQuery(async () => {
      const { data, error } = await this.client
        ...
        .insert({
          ...serviceData,
          barbershop_id: serviceData.barbershop_id || this.currentShopId
        })
        .select()
        .single()
  ...
  async getBusinessHours(shopId) {
    ...
    return executeQuery(async () => {
      // Business hours is a JSONB column in barbershops table, not a separate table
      const { data, error } = await this.client
        .from('barbershops')
      ...
      // Convert JSONB to array format expected by frontend
      if (data?.business_hours) {
        // If business_hours is already an array, return it
        ...
        return daysOfWeek.map((day, index) => ({
          day_of_week: index,
          day_name: day,
          is_open: data.business_hours[day]?.is_open || false,
          open_time: data.business_hours[day]?.open_time || '09:00',
          close_time: data.business_hours[day]?.close_time || '17:00'
        }))
      }
      
      // Return default business hours if none set
      return [
        { day_of_week: 0, day_name: 'monday', is_open: true, open_time: '09:00', close_time: '17:00' },
        { day_of_week: 1, day_name: 'tuesday', is_open: true, open_time: '09:00', close_time: '17:00' },
        { day_of_week: 2, day_name: 'wednesday', is_open: true, open_time: '09:00', close_time: '17:00' },
        { day_of_week: 3, day_name: 'thursday', is_open: true, open_time: '09:00', close_time: '17:00' },
        { day_of_week: 4, day_name: 'friday', is_open: true, open_time: '09:00', close_time: '17:00' },
        { day_of_week: 5, day_name: 'saturday', is_open: false, open_time: '09:00', close_time: '17:00' },
        { day_of_week: 6, day_name: 'sunday', is_open: false, open_time: '09:00', close_time: '17:00' }
      ]
    })
  }
  ...
  async getDashboardMetrics(shopId, options = {}) {
    ...
    return executeQuery(async () => {
      ...
      if (startDate) {
        appointmentQuery = appointmentQuery.gte('start_time', startDate)
      }
      if (endDate) {
        appointmentQuery = appointmentQuery.lte('start_time', endDate)
      }
