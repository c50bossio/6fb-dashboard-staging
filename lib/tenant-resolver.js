 ...
 * with a single, standardized approach that uses barberbarbershop_id consistently.
 * 
 ...
 * - profile.shop_id || profile.barbershop_id || (await getStaffShopId(profile.id))
...
export async function getTenant(userId, options = {}) {
  ...
  try {
    ...
    if (staffError && staffError.code !== 'PGRST116') {
      console.error('[TenantResolver] Staff query error:', staffError)
    }
...
export async function getUserBarbershop(userId, supabase) {
  ...
  return result.barberbarbershopId
}
 ...
 * @param {string} barberbarbershopId - Barbershop ID to check access to
 * @returns {Promise<{hasAccess: boolean, accessType: string, metadata: Object}>}
 */
export async function checkBarbershopAccess(userId, barberbarbershopId, options = {}) {
  ...
  if (!userId || !barberbarbershopId) {
    return {
      ...
      metadata: { error: 'Both userId and barberbarbershopId are required' }
    }
  }
  ...
  try {
    ...
    if (tenantResult.barberbarbershopId === barberbarbershopId) {
      return {
        hasAccess: true,
        accessType: 'direct_access',
        metadata: {
          source: tenantResult.source,
          resolvedBarberbarbershopId: tenantResult.barberbarbershopId
        }
      }
    }
    ...
    return {
      ...
      metadata: { 
        userBarberbarbershopId: tenantResult.barberbarbershopId,
        requestedBarberbarbershopId: barberbarbershopId
      }
    }
