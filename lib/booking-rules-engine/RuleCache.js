...
export class RuleCache {
  constructor(barberbarbershopId) {
    ...
    this.sessionStorageKey = `booking-rules-${barberbarbershopId}`
    this.localStorageKey = `booking-rules-backup-${barberbarbershopId}`
  }
   ...
   */
  isValid() {
    const memoryData = this.memoryCache.get(this.barberbarbershopId)
  ...
  async invalidate() {
    // Clear memory cache
    this.memoryCache.delete(this.barberbarbershopId)
   ...
   */
  getFromMemory() {
    const data = this.memoryCache.get(this.barberbarbershopId)
    if (!data) return null
   ...
   */
  setToMemory(data) {
    this.memoryCache.set(this.barberbarbershopId, {
...
class CacheManager {
  ...
  getCache(barberbarbershopId) {
    if (!this.caches.has(barberbarbershopId)) {
      this.caches.set(barberbarbershopId, new RuleCache(barberbarbershopId))
    }
    return this.caches.get(barberbarbershopId)
  }
  ...
  getStats() {
    const stats = {}
    for (const [barberbarbershopId, cache] of this.caches.entries()) {
      stats[barberbarbershopId] = cache.getStats()
    }
    return stats
  }
}
