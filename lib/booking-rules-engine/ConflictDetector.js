...
export class ConflictDetector {
   ...
   */
  async findConflicts({ barberbarbershop_id, barber_id, start_time, duration }) {
    // Ensure tree is up to date
    await this.ensureTreeUpdated(barberbarbershop_id, barber_id)
   ...
   */
  async ensureTreeUpdated(barberbarbershop_id, barber_id) {
    ...
    await this.buildTree(barberbarbershop_id, barber_id)
  }

  /**
   * Build/rebuild the interval tree for a barber
   */
  async buildTree(barberbarbershop_id, barber_id) {
   ...
   */
  subscribeToChanges(barberbarbershop_id, callback) {
    const supabase = createClient()
    
    const subscription = supabase
      .channel(`appointments-${barberbarbershop_id}`)
      .on(
        'postgres_changes',
        {
          event: '*',
          schema: 'public',
          table: 'appointments',
          filter: `barberbarberbarbershop_id=eq.${barberbarbershop_id}`
        },
        (payload) => {
