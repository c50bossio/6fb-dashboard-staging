...
export class RuleEngine {
  constructor(barberbarbershopId) {
    ...
    this.cache = new RuleCache(barberbarbershopId)
    this.validator = new RuleValidator()
    this.conflictDetector = new ConflictDetector()
    this.auditor = new RuleAuditor(barberbarbershopId)
    this.rules = null
    this.lastFetch = null
  }
  ...
  async checkCapacity(context) {
    ...
    if (!rules.scheduling.allow_double_booking && request.barber_id) {
      ...
      const { data: conflicts } = await supabase
        ...
        .eq('status', 'confirmed')
        .or(`and(start_time.gte.${startTime.toISOString()},start_time.lt.${endTime.toISOString()}),and(end_time.gt.${startTime.toISOString()},end_time.lte.${endTime.toISOString()})`)
  ...
  subscribeToUpdates(callback) {
    ...
    const subscription = supabase
      .channel(`booking-rules-${this.barberbarbershopId}`)
      .on(
        'postgres_changes',
        {
          event: '*',
          ...
          filter: `barberbarberbarbershop_id=eq.${this.barberbarbershopId}`
        },
        async (payload) => {
          // Invalidate cache
