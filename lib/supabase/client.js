    ...
    auth: {
      ...
      storage: {
        ...
        getItem: (key) => {
          ...
          try {
            // Fall back to cookies with base64 decoding
            ...
            if (cookieValue) {
              ...
              if (cookieValue.startsWith('base64-')) {
                try {
                  ...
                  console.log(`🔓 Decoded base64 cookie: ${key}`)
                  return decoded
                } catch (decodeError) {
                  console.warn(`⚠️ Base64 decode failed for ${key}:`, decodeError)
                  return cookieValue
                }
              }
              return cookieValue
          ...
          console.log(`❌ No storage found for: ${key}`)
          return null
        },
        setItem: (key, value) => {
          ...
          try {
            ...
            if (typeof window !== 'undefined' && window.localStorage) {
              ...
              const verifyItem = window.localStorage.getItem(key)
              if (verifyItem === value) {
                console.log(`✅ localStorage verification passed for: ${key}`)
              } else {
