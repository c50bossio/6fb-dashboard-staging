  ...
  client.getUserShopId = async function(userId) {
    try {
      ...
      if (profile?.shop_id || profile?.barbershop_id) {
        return profile.shop_id || profile.barbershop_id
      }
    ...
    cookies: {
      ...
      setAll(cookiesToSet) {
        try {
          ...
          dbLogger.warn('Failed to set cookie in server client', error, {
            cookie_name: name,
...
export async function createServiceRoleClient() {
  ...
  if (typeof window !== 'undefined') {
    dbLogger.warn('Attempted to use service role client on client-side, using regular client instead', null, {
  ...
  if (!SUPABASE_SERVICE_ROLE_KEY) {
    dbLogger.warn('Service role key not available, falling back to regular client', null, {
    ...
    cookies: {
      ...
      setAll(cookiesToSet) {
        try {
          ...
          dbLogger.warn('Failed to set service role cookie', error, {
            cookie_name: name,
...
export class SupabaseClientWrapper {
  ...
  async getCurrentUser() {
    try {
      ...
      if (error) {
        authLogger.error('Authentication error in getCurrentUser', error, {
      ...
      authLogger.error('Unexpected error in getCurrentUser', error, {
        context: this.context,
  ...
  async getUserBarbershopContext(userId = null) {
    try {
      ...
      dbLogger.error('Error getting user barbershop context', error, {
        context: this.context,
  ...
  async safeQuery(tableName, query, options = {}) {
    try {
      ...
      if (result.error) {
        if (logErrors) {
          dbLogger.error('Database query error', result.error, {
            context: this.context,
            table: tableName,
      ...
      dbLogger.error('Unexpected error in safeQuery', error, {
        context: this.context,
        table: tableName,
        operation: 'safeQuery'
      })
