...
// Lazy import of loggers to prevent circular dependency issues during authentication
let loggers = null
let loggersPromise = null
...
const getLoggers = () => {
  ...
  // Initialize console fallback immediately
  const fallbackLoggers = {
    dbLogger: { warn: console.warn, error: console.error, info: console.info },
    authLogger: { error: console.error, warn: console.warn, info: console.info }
  }
  
  // If we already started loading loggers, return fallback for now
  if (loggersPromise) {
    return fallbackLoggers
  }
  
  // Start loading loggers asynchronously
  loggersPromise = (async () => {
    try {
      const loggerModule = await import('@/lib/logger')
      loggers = {
        dbLogger: loggerModule.dbLogger,
        authLogger: loggerModule.authLogger
      }
    } catch (error) {
      // If logger import fails, keep using fallback
      loggers = fallbackLoggers
      console.warn('Logger module failed to load, using console fallback:', error.message)
    }
  })()
