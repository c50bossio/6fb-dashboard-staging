...
export async function createServiceRoleClient() {
  ...
  if (typeof window !== 'undefined') {
    console.warn('⚠️ Attempted to use service role client on client-side. Using regular client instead.')
    return createClient()
  }
    ...
    cookies: {
      ...
      setAll(cookiesToSet) {
        try {
          ...
          console.warn('Failed to set service role cookie:', name, error.message)
        }
      },
    },
  })
...
export class SupabaseClientWrapper {
  ...
  async getCurrentUser() {
    try {
      ...
      if (error) {
        console.error(`[${this.context}] Auth error:`, error.message)
        return { user: null, error }
      }

      return { user, error: null }
    } catch (error) {
      console.error(`[${this.context}] getCurrentUser error:`, error)
      return { user: null, error }
    }
  }
  ...
  async getUserProfile(userId = null) {
    try {
      ...
      const { data: profile, error } = await this.client
        ...
        .select(`
          *,
          barbershop:barbershops(id, name, owner_id),
          staff_associations:barbershop_staff(
            barberbarbershop_id,
            role,
            is_active,
      ...
      console.error(`[${this.context}] getUserProfile error:`, error)
  ...
  async getUserBarbershopContext(userId = null) {
    try {
      ...
      if (error) return { barberbarbershop_id: null, error }
      ...
      if (!barberbarbershop_id && profile?.staff_associations?.length > 0) {
        const activeAssociation = profile.staff_associations.find(assoc => assoc.is_active)
        barberbarbershop_id = activeAssociation?.barberbarbershop_id
      }
      ...
      console.error(`[${this.context}] getUserBarbershopContext error:`, error)
      return { barberbarbershop_id: null, error }
    }
  }
  ...
  async safeQuery(tableName, query, options = {}) {
    try {
      ...
      if (result.error) {
        if (logErrors) {
          console.error(`[${this.context}] Query error on ${tableName}:`, result.error)
        }
        if (throwOnError) {
          throw result.error
        }
      ...
      console.error(`[${this.context}] safeQuery error:`, error)
