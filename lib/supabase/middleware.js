...
export async function updateSession(request) {
  ...
  try {
    ...
    //
    // TIMEOUT PROTECTION: Wrap with 5-second timeout to prevent middleware hanging
    // CRITICAL SECURITY: On timeout, return null user to DENY access (fail-closed)
    // Protected routes will redirect to login, ensuring no unauthorized access
    const { data: { user }, error } = await withTimeout(
      supabase.auth.getUser(),
      5000, // 5 second timeout for better reliability
      'middleware-getUser'
    ).catch(timeoutError => {
      console.error('🚫 [Middleware] getUser() timed out - DENYING access:', timeoutError.message)
      // FAIL CLOSED: Return null user to block access on timeout
    ...
    if (error) {
      ...
      if (error.message && error.message.includes('timed out')) {
        console.error('🚫 [Middleware] Auth check timed out - access will be DENIED for protected routes')
      } else {
