...
export class ConversationRecoveryManager {
  ...
  checkForCrashRecovery() {
    try {
      ...
      if (recoveryData) {
        ...
        // Only recover if crash was recent
  ...
  acceptRecovery() {
    ...
    try {
      // Create a recovered conversation with special metadata
  ...
  importConversation(data, options = {}) {
    try {
      ...
      // Generate new session ID if conflict
  ...
  setupUnloadHandlers() {
    window.addEventListener('beforeunload', () => {
      // Clear recovery state on normal shutdown
      this.clearRecoveryState()
