  ...
  try {
    ...
    if (profile.role === 'BARBER') {
      ...
      if (error) {
        ...
        console.log('Found barbershop ID in barbershop_staff (employee barber):', staffData.barbershop_id)
        return staffData.barbershop_id
      }

      // If no staff record found, check if they have an active individual subscription
      console.log('No staff record found, checking for individual barber subscription')
      const subscription = await getUserSubscription(user.id)
      
      if (subscription && (subscription.tier === 'barber' || subscription.tier === 'shop' || subscription.tier === 'enterprise')) {
        console.log('Individual barber has active subscription:', subscription.tier)
        ...
        if (ownedShops?.id) {
          console.log('Found existing personal barbershop for individual barber:', ownedShops.id)
          
          // Ensure profile has shop_id set
          await supabase
            .from('profiles')
            .update({ shop_id: ownedShops.id })
            .eq('id', user.id)
          
          return ownedShops.id
        } else {
          // Create a personal barbershop for the individual barber
          console.log('Creating personal barbershop for individual barber with subscription')
          ...
          if (personalShop?.id) {
            console.log('Successfully created personal barbershop for individual barber:', personalShop.id)
            return personalShop.id
          }
        }
      } else {
        console.log('No active subscription found for individual barber')
      }
    ...
    if (profile.role === 'SHOP_OWNER' || profile.role === 'ENTERPRISE_OWNER') {
      console.log(`Looking up barbershop for ${profile.role} role`)
      ...
      if (error) {
        ...
        if (error.code === '406' || error.message?.includes('Not Acceptable')) {
          console.log('No barbershops found (expected during onboarding)')
          return null
        }
