 ...
 */
export async function getUserBarberbarbershopId(user, profile) {
  if (!user || !profile) {
    console.warn('getUserBarberbarbershopId: Missing user or profile data')
    return null
  }
  ...
  if (cached && (Date.now() - cached.timestamp) < CACHE_TTL) {
    ...
    return cached.barberbarbershopId
  }
  ...
  try {
    ...
    if (profile.barbershop_id) {
      ...
      console.warn('⚠️ Found legacy barbershop_id, should migrate to barberbarbershop_id:', profile.shop_id)
    } else {
      // Role-specific barbershop lookup
      barberbarbershopId = await getBarberbarbershopIdByRole(user, profile)
    }
    ...
    console.error('Error in getUserBarberbarbershopId:', error)
    return null
  }
}
 ...
 * @returns {Promise<string|null>} The barbershop ID or null
 */
async function getBarberbarbershopIdByRole(user, profile) {
  ...
  try {
    ...
    if (profile.role === 'BARBER') {
      ...
      } else if (staffData?.barberbarbershop_id) {
      ...
      if (subscription && (subscription.tier === 'barber' || subscription.tier === 'shop' || subscription.tier === 'enterprise')) {
        ...
        if (ownedShops?.id) {
          ...
          // FIXED: Update barberbarbershop_id field in unified schema
          await supabase
            .from('profiles')
            .update({ barberbarbershop_id: ownedShops.id })
            .eq('id', user.id)
    ...
    console.error('Error in getBarberbarbershopIdByRole:', error)
    return null
  }
}
...
export async function createBarbershopForOwner(user, shopData = {}) {
  ...
  try {
    ...
    if (existingShop) {
      ...
      await supabase
        .from('profiles')
        .update({ barberbarbershop_id: existingShop.id })
        .eq('id', user.id)
    ...
    const { error: profileError } = await supabase
      .from('profiles')
      .update({ barberbarbershop_id: barbershop.id })
      .eq('id', user.id)
 ...
 * @param {string} userId - The user ID
 * @param {string} barberbarbershopId - The barbershop ID
 * @param {string} role - The role in the barbershop
 * @returns {Promise<Object>} The staff association
 */
export async function associateBarberWithShop(userId, barberbarbershopId, role = 'BARBER') {
  const supabase = createClient()
