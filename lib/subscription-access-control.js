 ...
 */
export async function getUserSubscriptionTier(userId, barberbarbershopId = null) {
  try {
    // Get user data
    ...
    if (user.role === 'SHOP_OWNER' && barberbarbershopId) {
 ...
 */
export async function checkReviewFeatureAccess(userId, feature, barberbarbershopId = null) {
  try {
    const subscription = await getUserSubscriptionTier(userId, barberbarbershopId)
  ...
  try {
    ...
    if (subscription.user_role === 'ENTERPRISE_OWNER') {
      ...
      const { data: workLocations } = await supabase
        ...
        .select('barberbarbershop_id, barbershops(id, name)')
        .eq('user_id', userId)
        .eq('is_active', true)
 ...
 */
export async function getFilteredReviewData(userId, barberbarbershopId, options = {}) {
  try {
    const subscription = await getUserSubscriptionTier(userId, barberbarbershopId)
    const locationAccess = await checkLocationAccess(userId, [barberbarbershopId])
