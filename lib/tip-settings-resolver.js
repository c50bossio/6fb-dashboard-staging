 ...
 * @param {string} params.barberbarbershopId - The barbershop ID
 ...
 */
export async function resolveTipSettings({ barberbarbershopId, barberId, serviceId }) {
  ...
  try {
    ...
    if (serviceId && barberbarbershopId) {
...
export async function getTipSettingsForCheckout(checkoutContext) {
  const { barberbarbershopId, barberId, serviceId, serviceAmount } = checkoutContext
  
  // Resolve the settings based on hierarchy
  const settings = await resolveTipSettings({ barberbarbershopId, barberId, serviceId })
