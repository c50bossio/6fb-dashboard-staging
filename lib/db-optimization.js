...
export async function loadWithRelations(table, options = {}) {
  ...
  // Attach related data to main records
...
class ConnectionPool {
  ...
  createConnection() {
    // Return a wrapped supabase client with automatic release
    return {
      query: async (fn) => {
        try {
...
export async function aggregateQuery(table, options = {}) {
  ...
  // Apply having clause in memory (Supabase doesn't support HAVING directly)
...
// Start prefetcher on module load
