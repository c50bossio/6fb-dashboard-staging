 ...
 */
export async function checkGMBRateLimit(barberbarbershopId, locationId = null) {
  const checks = []
...
export async function logGMBApiUsage({
  ...
  try {
    ...
    const statsKey = `gmb:stats:${dateKey}:${barberbarbershopId}`
 ...
 */
export async function getGMBUsageStats(barberbarbershopId, days = 7) {
  const client = redis || memoryLimiter
 ...
 */
export async function checkQuotaHealth(barberbarbershopId) {
  try {
    const dailyLimit = await checkRateLimit('DAILY_QUOTA')
    const burstLimit = await checkRateLimit('MINUTE_BURST')
    const usageStats = await getGMBUsageStats(barberbarbershopId, 1)
