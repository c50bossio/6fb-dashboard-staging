...
export const formatPhoneNumber = (value) => {
  ...
  // Default: return first 10 digits formatted
  return `(${phoneNumber.slice(0, 3)}) ${phoneNumber.slice(3, 6)}-${phoneNumber.slice(6, 10)}`
}
...
export const formatInternationalPhone = (value) => {
  ...
  // For other international numbers, add spaces every 3-4 digits
...
export const autoFormat = (value, inputType, inputName, placeholder) => {
  ...
  // Determine formatter based on input attributes
  ...
  if (inputType === 'tel' || 
    ...
    // Check if it looks like international format
