...
export async function getCurrentUser() {
  ...
  try {
    ...
    // If no session exists, return null (this is normal for unauthenticated users)
    if (!session || sessionError) {
      supabaseLogger.debug('No active session')
      return null
    }
    ...
    // Only log as error if it's an actual error, not just missing session
