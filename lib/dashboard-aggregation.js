...
import { getDisplayName, normalizeNameData } from './name-utils'
...
async function fetchLocationMetrics(locationId, timeRange) {
  try {
    ...
    const revenue = appointments?.reduce((sum, apt) => sum + (apt.total_price_cents ? apt.total_price_cents / 100 : 0), 0) || 0
...
export async function getFilteredCalendarEvents(locationIds, barberIds, timeRange) {
  try {
    ...
    // Transform to calendar event format with enhanced name handling
    return data?.map(apt => {
      // Normalize barber name data
      const barberNormalizedNames = apt.profiles ? normalizeNameData({
        firstName: apt.profiles.first_name,
        lastName: apt.profiles.last_name,
        fullName: apt.profiles.full_name
      }) : {}
      
      const barberDisplayName = apt.profiles ? getDisplayName({
        firstName: barberNormalizedNames.firstName,
        lastName: barberNormalizedNames.lastName,
        fullName: barberNormalizedNames.fullName,
        email: apt.profiles.email,
        defaultName: 'Unknown Barber'
      }) : 'Unknown Barber'

      return {
        id: apt.id,
        title: apt.title || `${apt.services?.name || 'Appointment'} - ${apt.customers?.full_name || 'Customer'}`,
        ...
        extendedProps: {
          ...
          barberFirstName: barberNormalizedNames.firstName,
          barberLastName: barberNormalizedNames.lastName,
          barberFullName: barberNormalizedNames.fullName,
