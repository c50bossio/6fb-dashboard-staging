-- Unified AI Tables for Supabase PostgreSQL (No pgvector)
...
-- Function to search knowledge documents using JSONB similarity (basic)
...
BEGIN
  ...
  SELECT 
    ...
    (
      CASE 
        WHEN kd.content ILIKE '%' || search_text || '%' THEN 1.0
        WHEN kd.title ILIKE '%' || search_text || '%' THEN 0.8
        ELSE ts_rank(
  ...
  WHERE kd.is_active = true
    ...
    AND (
      ...
      kd.title ILIKE '%' || search_text || '%' OR
      to_tsvector('english', kd.content) @@ to_tsquery('english', search_text)
    )
...
-- WITHOUT pgvector dependency - uses JSONB for embeddings and text search for similarity