...
-- Check if the user exists in profiles table (may be using 'users' table name)
SELECT 
  id,
  email,
  role,
...
-- If profiles table doesn't exist, check users table
SELECT 
  id,
  email,
  role,
...
-- If that didn't work, try users table
UPDATE users 
SET 
  role = 'SHOP_OWNER',
  subscription_status = 'active',
...
UNION ALL
SELECT 
  'User Profile (users table)' as check_type,
  email,
  role,
  barbershop_id,
  shop_id
