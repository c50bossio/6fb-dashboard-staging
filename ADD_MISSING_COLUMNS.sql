-- ===============================================
-- ADD MISSING COLUMNS TO MARKETING TABLES
-- Run this in Supabase SQL Editor after the initial tables are created
-- ===============================================

-- Add missing columns to marketing_accounts table
...
ADD COLUMN IF NOT EXISTS daily_send_limit INTEGER DEFAULT 10000,
ADD COLUMN IF NOT EXISTS require_approval_above DECIMAL(10,2) DEFAULT 100.00,
ADD COLUMN IF NOT EXISTS authorized_users TEXT[],
ADD COLUMN IF NOT EXISTS verification_method TEXT,
ADD COLUMN IF NOT EXISTS verified_at TIMESTAMPTZ,
ADD COLUMN IF NOT EXISTS include_unsubscribe_link BOOLEAN DEFAULT true,
ADD COLUMN IF NOT EXISTS include_company_address BOOLEAN DEFAULT true,
ADD COLUMN IF NOT EXISTS company_address TEXT,
ADD COLUMN IF NOT EXISTS gdpr_compliant BOOLEAN DEFAULT true;

-- Create marketing_billing_records table if it doesn't exist properly
CREATE TABLE IF NOT EXISTS marketing_billing_records (
    id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
    billing_account_id UUID REFERENCES marketing_accounts(id) ON DELETE CASCADE NOT NULL,
    campaign_id UUID,
    invoice_id TEXT,
...
-- Create marketing_campaigns table for tracking campaigns
...
CREATE POLICY "Users can view billing records for their accounts" ON marketing_billing_records
    FOR SELECT USING (
        EXISTS (
            ...
            WHERE marketing_accounts.id = marketing_billing_records.billing_account_id 
