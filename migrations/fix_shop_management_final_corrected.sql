-- ==========================================
-- Shop Management Module - Final Corrected Migration
-- ==========================================
-- This is the definitive migration that handles all discovered issues:
-- 1. Mixed data types (UUID, TEXT, VARCHAR)
-- 2. Views vs Tables (appointments is a VIEW)
-- 3. Type casting for auth.uid() comparisons
-- 4. Correct column names (start_time vs booking_date)
-- 5. Missing tables handled gracefully
...
-- ==========================================
-- PART 3: BOOKINGS TABLE (TEXT shop_id, TEXT customer_id)
-- ==========================================
...
BEGIN
  ...
  IF EXISTS (
    ...
    -- Create new policies - shop_id and customer_id are TEXT in bookings table
    ...
    -- Fixed policy: customer_id is TEXT, auth.uid() returns UUID, so we cast UUID to TEXT
...
-- Create comprehensive dashboard view that handles mixed data types and correct column names
...
SELECT 
  ...
  -- Booking metrics (handles TEXT shop_id and uses start_time)
  COALESCE((
    SELECT COUNT(*) 
    FROM bookings bk 
  ...
  -- Revenue metrics (UUID barbershop_id) - using booking price
  COALESCE((
    SELECT SUM(bk.price) 
    FROM bookings bk 
    WHERE bk.shop_id::text = b.id::text 
...
-- Function to calculate shop metrics (handles mixed types and correct column names)
...
BEGIN
  ...
  RAISE NOTICE '  All column names verified and corrected';
  RAISE NOTICE '  Status: SUCCESS';
END $$;

SELECT 'Shop Management Final Corrected Migration Complete - All issues resolved!' as status;