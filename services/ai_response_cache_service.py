...
class AIResponseCacheService:
    ...
    def __init__(self, strategy: CacheStrategy = CacheStrategy.BALANCED):
        ...
        # Initialize Redis connection lazily (avoid asyncio issues)
...
# Initialize these lazily to avoid asyncio issues
ai_cache_aggressive = None
ai_cache_balanced = None
ai_cache_conservative = None
ai_response_cache_service = None

def get_cache_service(strategy: CacheStrategy = CacheStrategy.BALANCED) -> AIResponseCacheService:
    """Get cache service instance for given strategy"""
    global ai_cache_aggressive, ai_cache_balanced, ai_cache_conservative, ai_response_cache_service
    
    if strategy == CacheStrategy.AGGRESSIVE:
        if ai_cache_aggressive is None:
            ai_cache_aggressive = AIResponseCacheService(CacheStrategy.AGGRESSIVE)
        return ai_cache_aggressive
    elif strategy == CacheStrategy.CONSERVATIVE:
        if ai_cache_conservative is None:
            ai_cache_conservative = AIResponseCacheService(CacheStrategy.CONSERVATIVE)
        return ai_cache_conservative
    else:  # BALANCED
        if ai_cache_balanced is None:
            ai_cache_balanced = AIResponseCacheService(CacheStrategy.BALANCED)
        if ai_response_cache_service is None:
            ai_response_cache_service = ai_cache_balanced
        return ai_cache_balanced
...
async def get_cached_response(message: str, context: Dict = None, provider: str = "", strategy: CacheStrategy = CacheStrategy.BALANCED) -> Optional[Dict]:
    ...
    cache_service = get_cache_service(strategy)
