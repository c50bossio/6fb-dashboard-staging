-- =====================================================
-- COMPLETE FIX WITH EXTENDED FIELD LENGTHS
-- Run this SQL in Supabase SQL Editor
-- =====================================================

-- 1. ADD ALL MISSING COLUMNS TO CUSTOMERS TABLE (WITH LONGER FIELDS)
-- =====================================================
ALTER TABLE customers 
ADD COLUMN IF NOT EXISTS barbershop_id VARCHAR(255),
ADD COLUMN IF NOT EXISTS last_visit_at TIMESTAMP WITH TIME ZONE,
ADD COLUMN IF NOT EXISTS referral_code VARCHAR(50),  -- Increased from 20
...
-- Fix phone column length if it exists
ALTER TABLE customers 
ALTER COLUMN phone TYPE VARCHAR(50);  -- Increased to 50 for international numbers
...
-- 2. ADD MISSING COLUMNS TO BOOKINGS TABLE (WITH LONGER FIELDS)
-- =====================================================
ALTER TABLE bookings
ADD COLUMN IF NOT EXISTS customer_name VARCHAR(255),
ADD COLUMN IF NOT EXISTS customer_phone VARCHAR(50),  -- Increased to 50
...
BEGIN
  ...
  IF TG_OP = 'INSERT' AND NEW.customer_id IS NOT NULL THEN
    ...
    -- Copy customer data to booking (with proper length handling)
    UPDATE bookings b
    SET 
      customer_name = LEFT(c.name, 255),
      customer_phone = LEFT(c.phone, 50),
      customer_email = LEFT(c.email, 255)
    FROM customers c
    WHERE b.id = NEW.id 
  ...
  ELSIF TG_OP = 'UPDATE' THEN
    IF OLD.customer_id IS DISTINCT FROM NEW.customer_id THEN
      ...
      IF NEW.customer_id IS NOT NULL THEN
        ...
        -- Update customer data in booking with length handling
        UPDATE bookings b
        SET 
          customer_name = LEFT(c.name, 255),
          customer_phone = LEFT(c.phone, 50),
...
-- 6. UPDATE EXISTING CUSTOMER STATS (SAFE VERSION)
-- =====================================================
...
-- Copy customer data to bookings (with length protection)
UPDATE bookings b
SET 
  customer_name = LEFT(c.name, 255),
  customer_phone = LEFT(c.phone, 50),
...
-- Phone fields now support up to 50 characters
-- All data is safely truncated if needed
-- =====================================================