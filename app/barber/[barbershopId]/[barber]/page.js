...
export async function generateMetadata({ params }) {
  try {
    ...
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:9999'}/api/public/barber/${barberbarbershopId}/${barber}`,
      { cache: 'no-store' }
    )
    ...
    return {
      ...
      alternates: {
        canonical: `${process.env.NEXT_PUBLIC_APP_URL}/barber/${barberbarbershopId}/${barber}`
      }
    }
  } catch (error) {
