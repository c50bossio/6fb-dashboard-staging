...
// Import the advanced appointment booking modal with full features
...
export default function CalendarPage() {
  ...
  const handleSlotClick = useCallback((slotInfo) => {
    ...
    console.log('📅 [PAGE] Setting modal open to true')
    setSelectedSlot(slotInfo)
    setEditingAppointment(null)
    setIsModalOpen(true)
    console.log('📅 [PAGE] Modal state should now be: true')
  }, [])
  ...
  const handleEventClick = useCallback((clickInfo) => {
    ...
    // Set up slot info from the existing appointment
    setSelectedSlot({
  ...
  return (
    <div className="p-4 sm:p-6 max-w-[1600px] mx-auto">
      ...
      <div className="mb-6 bg-olive-600 rounded-xl p-6 text-white shadow-md border border-olive-700">
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
          ...
          <div className="flex flex-wrap items-center gap-3 lg:gap-4">
            ...
            {/* TEST BUTTON - Click to verify modal can open */}
            <button
              onClick={() => {
                console.log('🧪 [TEST] Test button clicked - opening modal manually')
                setSelectedSlot({ start: new Date(), end: new Date(), duration: 60 })
                setEditingAppointment(null)
                setIsModalOpen(true)
              }}
              className="bg-yellow-500/80 hover:bg-yellow-600 px-4 py-3 rounded-lg transition-colors text-white font-semibold text-sm"
            >
              TEST MODAL
            </button>
          </div>
      ...
      {/* Advanced Appointment Booking Modal with full features */}
          ...
          <div className="flex-1">
            ...
            <ul className="text-sm text-olive-800 dark:text-olive-300 space-y-2.5">
              ...
              <li className="flex items-start gap-3">
                ...
                <span className="leading-relaxed">Click on existing appointments to view or edit details</span>
              </li>
              <li className="flex items-start gap-3">
                <svg className="h-5 w-5 text-brand-600 dark:text-brand-500 mt-0.5 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                  <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" />
