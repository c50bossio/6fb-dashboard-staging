-- =====================================================
-- Complete Inventory Marketplace Deployment
...
-- Drop existing tables if you want a clean install (OPTIONAL - COMMENT OUT IF YOU WANT TO KEEP DATA)
-- DROP TABLE IF EXISTS marketplace_order_items CASCADE;
-- DROP TABLE IF EXISTS marketplace_orders CASCADE;
-- DROP TABLE IF EXISTS marketplace_enrollment CASCADE;
-- DROP TABLE IF EXISTS inventory_movements CASCADE;
-- DROP TABLE IF EXISTS inventory_alerts CASCADE;
-- DROP TABLE IF EXISTS barbershop_inventory CASCADE;
...
-- Add computed column for available quantity (only if it doesn't exist)
DO $$ 
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'barbershop_inventory' 
...
-- Add computed column for available credit (only if it doesn't exist)
DO $$ 
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'marketplace_enrollment' 
...
-- Add computed column for balance due (only if it doesn't exist)
DO $$ 
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
...
-- =====================================================
-- INVENTORY ALERTS (with fix for missing columns)
-- =====================================================
...
SELECT sku, name, wholesale_price, msrp 
FROM master_products 
WHERE sku LIKE 'BB-%'
ORDER BY sku;
...
BEGIN
  ...
  RAISE NOTICE '📦 Tables created: 8 inventory tables';
  RAISE NOTICE '🛍️ Sample products: 5 BookedBarber products added';
  RAISE NOTICE '🔐 Ready for RLS policies (run separate script if needed)';
END $$;