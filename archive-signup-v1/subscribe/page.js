...
export default function SubscribePage() {
  ...
  const handleSelectPlan = async (tierId) => {
    ...
    try {
      ...
      // NEW FLOW: User not authenticated - redirect to registration page with plan data
      console.log('🔒 Redirecting to registration page with plan selection')
      
      // Redirect to registration page with plan data as URL parameters
      router.push(`/register?plan=${tierId}&billing=${billingPeriod}`)
      
      // The registration page will handle authentication options:
      // 1. User can choose email/password OR Google OAuth
      // 2. Both paths will preserve plan data and redirect to Stripe
              ...
              >
                {authLoading ? (
                  ...
                  <span className="flex items-center justify-center">
                    ...
                    {!user ? 'Redirecting to sign up...' : 'Processing...'}
                  </span>
                ) : (
                  tier.cta
                )}
