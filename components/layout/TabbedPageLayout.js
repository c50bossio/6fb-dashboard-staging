    ...
    <div className={`space-y-6 ${fullWidth ? '' : 'max-w-7xl mx-auto px-4 sm:px-6 lg:px-8'}`}>
        ...
        <div className="px-6 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              {TitleIcon && (
                <div className="flex items-center justify-center w-10 h-10 bg-gradient-to-br from-olive-500 to-gold-600 rounded-lg">
          ...
          <Card className="bg-olive-50 border-olive-200">
            <div className="p-4">
              <div className="flex items-start space-x-3">
                {activeTabData.icon && (
                  <activeTabData.icon className="h-5 w-5 text-olive-600 mt-0.5" />
                )}
                <div>
                  <h3 className="text-sm font-medium text-olive-900">
                    {activeTabData.name}
                  </h3>
                  ...
                  {activeTabData.features && activeTabData.features.length > 0 && (
                    <ul className="mt-2 text-sm text-olive-600 space-y-1">
                      {activeTabData.features.map((feature, index) => (
                        <li key={index} className="flex items-center">
                          <div className="w-1 h-1 bg-olive-500 rounded-full mr-2"></div>
                          {feature}
                        </li>
                      ))}
                    </ul>
              ...
              <div className="text-center py-12">
                ...
                <div className="mt-6">
                  <button
                    ...
                    className="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md text-white bg-olive-600 hover:bg-olive-700"
                  >
      ...
      <div className="lg:hidden">
        <select
          ...
          className="block w-full border-gray-300 rounded-md shadow-sm focus:ring-olive-500 focus:border-olive-500"
        >
          {tabs.map((tab) => (
  ...
  ComingSoon: ({ title = "Coming Soon", description = "This feature is under development.", icon: Icon }) => (
    <div>
      <div className="inline-flex items-center justify-center w-16 h-16 bg-olive-100 rounded-full mb-4">
        {Icon ? <Icon className="w-8 h-8 text-olive-600" /> : (
