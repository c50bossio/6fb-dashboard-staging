...
export default function StaffSetup({ data = {}, updateData, onComplete }) {
  ...
  // Initialize staff from session data, fallback to prop data
  const [staff, setStaff] = useState(() => {
    return sessionData?.stepData?.staff || data.staff || []
  })
  ...
  // Auto-save to Supabase when staff changes
  const debouncedSave = useCallback(
    debounce((staffData) => {
      saveStep('staff_list', { staff: staffData })
    }, 1000),
    [saveStep]
  )

  useEffect(() => {
    // Update parent data (maintain backwards compatibility)
    if (updateData) {
      updateData({ staff })
    }
    
    // Auto-save to Supabase database
    if (staff.length > 0 || sessionData) {
      debouncedSave(staff)
    }
  }, [staff, updateData, debouncedSave, sessionData])
  
  // Restore staff data when session data changes (cross-tab sync)
  useEffect(() => {
    if (sessionData?.stepData?.staff && !hasLocalChanges) {
      setStaff(sessionData.stepData.staff)
    }
  }, [sessionData?.stepData?.staff, hasLocalChanges])

  // Handle completion with Supabase persistence
  const handleComplete = async () => {
    try {
      // Save final data
      await saveStep('staff_list', { staff, completed: true })
      
      // Mark step as completed in progress tracking
      await markStepComplete('staff_list')
    ...
    <div className="space-y-6">
      ...
      {saveStatus === 'saving' && (
        <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3 mb-4">
          <div className="flex items-center">
            ...
            <span className="text-sm text-yellow-800">Saving changes...</span>
          </div>
        </div>
      )}
      ...
      {saveStatus === 'offline' && (
        <div className="bg-orange-50 border border-orange-200 rounded-lg p-3 mb-4">
          <div className="flex items-center">
            <ExclamationCircleIcon className="h-4 w-4 mr-2 text-orange-600" />
            <span className="text-sm text-orange-800">Changes saved offline - will sync when reconnected</span>
          </div>
        </div>
      )}
      
      {saveStatus === 'saved' && hasUnsavedChanges === false && staff.length > 0 && (
        <div className="bg-green-50 border border-green-200 rounded-lg p-3 mb-4">
          <div className="flex items-center">
            <CheckCircleIcon className="h-4 w-4 mr-2 text-green-600" />
            <span className="text-sm text-green-800">All changes saved automatically</span>
          </div>
        </div>
      )}
