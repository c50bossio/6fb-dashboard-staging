...
export default function SmartAlertsPanel({ barbershop_id = 'demo' }) {
  ...
  const getAlertIcon = (type) => {
    switch (type) {
      ...
      case 'info': return <InformationCircleIcon className="h-5 w-5 text-olive-500 dark:text-olive-400" />
      case 'success': return <CheckCircleIcon className="h-5 w-5 text-green-500 dark:text-green-400" />
  ...
  const getAlertBorderColor = (type) => {
    switch (type) {
      ...
      case 'info': return 'border-l-blue-500 bg-olive-50 dark:bg-olive-900/20'
      case 'success': return 'border-l-green-500 bg-green-50 dark:bg-green-900/20'
      default: return 'border-l-gray-500 bg-muted/50'
    }
  }
  ...
  return (
    <div className="space-y-6">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {alert.recommendation && (
                        <div className="mt-3 flex items-start space-x-2">
                          <LightBulbIcon className="h-4 w-4 text-amber-700 dark:text-amber-400 mt-0.5" />
                          <div>
                      ...
                      {alert.action_required && (
                        <button className="mt-3 text-sm text-olive-600 dark:text-olive-400 hover:text-indigo-800 dark:hover:text-indigo-400 font-medium flex items-center">
      ...
      {priorityActions.length > 0 && (
        <div className="bg-card rounded-lg shadow-sm border border-border">
          <div className="p-4 border-b border-border">
            <div className="flex items-center space-x-2">
              <ClockIcon className="h-5 w-5 text-olive-500 dark:text-olive-400" />
            ...
            {priorityActions.map((action, idx) => (
              <div key={idx} className="p-4 flex items-center justify-between hover:bg-muted/50 transition-colors">
                <div className="flex-1">
                  ...
                  {action.deadline && (
                    <p className="text-xs text-muted-foreground/70 mt-1">Due: {action.deadline}</p>
                  )}
                </div>
      ...
      {alerts.length === 0 && priorityActions.length === 0 && !loading && (
        <div className="bg-card rounded-lg shadow-sm border border-border p-8 text-center">
          <CheckCircleIcon className="h-12 w-12 text-green-500 dark:text-green-400 mx-auto mb-3" />
          <h3 className="text-lg font-semibold text-foreground">All Clear!</h3>
          <p className="text-muted-foreground mt-1">No alerts or priority actions at the moment.</p>
        </div>
      )}
    </div>
  )
