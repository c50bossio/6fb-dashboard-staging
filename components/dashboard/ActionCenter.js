...
export default function ActionCenter({ data }) {
  ...
  return (
    <div className="space-y-6">
      ...
      {hasCurrentActivity() && (
        <div className="bg-gradient-to-r from-green-50 to-olive-50 dark:from-green-900/20 dark:to-olive-900/20 border border-green-200 dark:border-green-800 rounded-xl p-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="h-3 w-3 bg-green-500 rounded-full animate-pulse"></div>
              <div>
                <h3 className="font-semibold text-foreground">Current Activity</h3>
                <p className="text-sm text-muted-foreground mt-1">{getRealtimeStatus()}</p>
              </div>
            </div>
            <div className="flex items-center gap-4 text-sm">
              <div className="flex items-center gap-2">
                <ClockIcon className="h-4 w-4 text-muted-foreground" />
                <span className="text-muted-foreground">Next available: {realtime.next_available || '11:30 AM'}</span>
              </div>
              <button className="p-2 rounded-lg bg-card text-muted-foreground hover:bg-muted transition-colors">
      ...
      <div className="bg-card rounded-xl shadow-sm border border-border p-6">
        <div className="flex items-center justify-between mb-6">
          <h3 className="text-lg font-semibold text-foreground flex items-center gap-2">
            <ClipboardDocumentCheckIcon className="h-6 w-6 text-green-500 dark:text-green-400" />
      ...
      {alerts.length > 0 && (
        <div className="bg-card rounded-xl shadow-sm border border-border p-6">
          <h3 className="text-lg font-semibold text-foreground mb-4 flex items-center gap-2">
            <BellAlertIcon className="h-6 w-6 text-amber-700 dark:text-amber-400" />
            Active Alerts
          </h3>
      ...
      <div className="bg-card rounded-xl shadow-sm border border-border p-6">
        ...
        {appointments.length > 5 && (
          <button className="w-full mt-4 py-2 text-sm font-medium text-olive-600 dark:text-olive-400 hover:text-olive-800 dark:hover:text-olive-300 border-t border-border pt-4">
...
const TaskCard = ({ task, isActive, isCompleted, onAction }) => {
  ...
  const priorityColors = {
    ...
    low: 'border-olive-200 dark:border-olive-800 bg-olive-50 dark:bg-olive-900/20'
  }

  if (isCompleted) {
    return (
      <div className="p-4 rounded-lg border border-border bg-muted opacity-60">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
          ...
          <div className="flex-1">
            ...
            <div className="flex items-center gap-3 mt-2 text-xs text-muted-foreground">
              ...
              <span className={`px-2 py-0.5 rounded-full font-medium ${
                task.priority === 'high' ? 'bg-softred-100 dark:bg-softred-900/30 text-softred-800 dark:text-softred-200' :
          ...
          {isActive ? (
            ...
            task.actions.map((action, index) => (
              <button
                ...
                className="px-3 py-1 bg-card border border-border rounded-lg text-sm text-foreground hover:bg-muted transition-colors"
              >
                {action}
              </button>
            ))
...
const AlertCard = ({ alert }) => {
  const typeStyles = {
    ...
    info: 'bg-olive-50 dark:bg-olive-900/20 border-olive-200 dark:border-olive-800 text-olive-800 dark:text-olive-200',
...
const QuickActionButton = ({ icon: Icon, label, color, badge, onClick }) => (
  <button
    onClick={onClick}
    className={`relative p-4 bg-card rounded-xl shadow-sm border border-border hover:shadow-md transition-all hover:border-${color}-300`}
  >
    {badge && (
...
const AppointmentCard = ({ appointment }) => (
  <div className="flex items-center justify-between p-3 rounded-lg border border-border hover:bg-muted transition-colors">
    <div className="flex items-center gap-3">
      <div className="text-sm font-bold text-muted-foreground w-16">
    ...
    <div className="flex items-center gap-2">
      <span className={`w-2 h-2 rounded-full ${
        appointment.status === 'confirmed' ? 'bg-green-500 dark:bg-green-400' :
        appointment.status === 'pending' ? 'bg-amber-500 dark:bg-amber-400' :
        'bg-muted-foreground'
      }`} />
